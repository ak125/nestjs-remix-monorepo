...
export default function VehicleSelectorGamme({
  ...
  const handleTypeChange = (typeId: number) => {
    ...
    if (type && selectedBrand && selectedModel) {
      // 🚗 Navigation vers page véhicule constructeur (pas gamme)
      const brandSlug = selectedBrand.marque_alias || selectedBrand.marque_name.toLowerCase().replace(/\s+/g, '-');
      const modelSlug = selectedModel.modele_alias || selectedModel.modele_name.toLowerCase().replace(/\s+/g, '-');
      const typeSlug = type.type_alias || type.type_name.toLowerCase().replace(/\s+/g, '-');
      
      const vehicleUrl = `/constructeurs/${brandSlug}/${modelSlug}/${typeSlug}.html`;
      console.log('🚗 Navigation VehicleSelectorGamme vers:', vehicleUrl);
      navigate(vehicleUrl);
    }
  };
