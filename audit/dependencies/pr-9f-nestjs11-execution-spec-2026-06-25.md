...
## IMPLEMENTATION (PR-9f réel — 2026-06-25)

Écarts vs plan, découverts à l'exécution et tranchés sur preuve :

1. **Réconciliation overlay repliée dans PR-9f.** `#1144` (PR A) n'a touché QUE les 2 `.md`
   d'audit — la réconciliation `family-overlay.yaml` (item PR A.2) n'a jamais été faite. Repliée
   ici : `staging_soak_hours 72→24`, `node_runtime_requirement ">=20"→">=24"`,
   `estimated_canary_duration_minutes 60→0`, `deployment_sequence [canary, full-rollout]→[full-rollout]`
   (`atomic` rejeté : hors enum Zod du builder d'inventaire → `full-rollout` = déploiement atomique 1-phase).
2. **`@nestjs/bull` + `@nestjs/passport` = bumps cross-famille peer-forcés (chirurgicaux, hors outil).**
   Le bump gouverné `bump-dependency-family --family runtime-backend-nest` ne couvre PAS ces 2 wrappers
   (membres de `queues-bullmq` / `auth-session-passport`). Or `@nestjs/bull@10` (peer `^8||^9||^10`) et
   `@nestjs/passport@10` (peer `^8||^9||^10`) cassent le boot sous Nest 11. Bump chirurgical minimal
   `@nestjs/bull ^11.0.4` + `@nestjs/passport ^11.0.5` — **moteurs `bull@4.16.5` / `passport@0.7.0`
   INCHANGÉS** (pr-9d / pr-9e préservés). Reclassification de famille rejetée (sur-portée gouvernance
   d'autres PRs ; blast-radius minimal préféré).
3. **Bumps majeurs annexes alignés Express 5 / Nest 11** (via l'outil gouverné, famille déjà membre) :
   `body-parser ^1.20.3→^2.3.0` (Express 5 ; `main.ts` passe `extended: true` explicite → 0 changement),
   `reflect-metadata` root `^0.1.14→^0.2.2` (Nest 11 exige `^0.2`). **`rxjs` inchangé (7.8.2)**,
   **`@nestjs/cache-manager` reste 3.x (3.1.3)** — pont v6/Keyv de PR-9f.cache protégé.
4. **Query parser** : `expressApp.set('query parser', 'extended')` à `main.ts` (à côté de `trust proxy`),
   rétablit le défaut Express 4 (objets/tableaux imbriqués) que Express 5 a changé en `simple`.
5. **`@SplatPath()` = '' à la racine (pas '/')**, les sites metadata/breadcrumb préfixent `'/' + path`,
   les sites seo l'utilisent nu — préserve les 2 formes (avec/sans slash de tête) à l'octet près.

### Preuves de vérification (local DEV, avant merge)
- `tsc --noEmit` **0 erreur** ; `tsc --build` (emit `dist/main.js`) **0 erreur**.
- Caractérisation routes (`route-splat-characterization.test.ts`, 16 cas) **identique sous Nest 10 et
  Nest 11** — contrat d'URL réel (single-encodé) préservé. Seul écart : double-encodé `a%252Fb` passe de
  `/a/b` (double-décode legacy) à `/a%2Fb` (single-décode) — **durcissement CWE-174 intentionnel**, posé
  au swap `@SplatPath` (C1).
- Enregistrement + scoping des 5 middleware wildcard (`route-wildcard-middleware.test.ts`, path-to-regexp 8) :
  `{*path}` matche racine+profond ; `api/diagnostic/*path` matche son préfixe, **pas** un frère.
- Tree : `express@5.2.1`, `path-to-regexp@8.4.2`, `@nestjs/bull-shared@11.0.4`, **0 `@nestjs/*@10`**,
  **0 invalid peer**, override `path-to-regexp 3.3.0` retiré, `swagger-ui-express` retiré (×2).
- Suite backend complète : **215 suites / 2540 tests verts** ; 18 suites en échec = baseline `SUPABASE_URL`
  (env-only, inchangée) ; **0 nouvelle régression** Nest 11.
- ast-grep : `backend-no-legacy-route-wildcard` (erreur, catch 5 formes legacy, 0 faux-positif sur
  `.select('*')`/`{*path}`/`*path`) + `backend-no-decode-in-splat-controllers` (erreur, file-scopé) — **0 finding**.
- Inventaire modernization régénéré + déterministe (gate BLOQUANT).

**Restant = container PREPROD** (env + Supabase + Redis) : boot complet, Swagger `/api/docs` 200, WebSocket,
worker `:3001`, login/session, SSR — couverts par E2E Smoke + Lighthouse CI post-merge. Tag PROD `v*` = **GO owner**.
