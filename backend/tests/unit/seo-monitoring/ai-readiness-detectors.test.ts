...
} from '../../../src/modules/seo-monitoring/helpers/ai-readiness-detectors';

describe('detectExtractableTldr', () => {
  it('returns 1 when first <p> has 50-200 chars of substantive text', () => {
    const html =
      '<html><body><h1>Symptôme</h1><p>La fumée noire au démarrage indique souvent un encrassement de la vanne EGR sur Clio 1.5 dCi.</p></body></html>';
    expect(detectExtractableTldr(html)).toBe(1);
  });
  it('returns 0 when no <p> in first 1000 chars', () => {
    const html =
      '<html><body><h1>Symptôme</h1><div>Texte sans paragraphe.</div></body></html>';
    expect(detectExtractableTldr(html)).toBe(0);
  });
  it('returns 0 when first <p> is too short (<50 chars)', () => {
    expect(
      detectExtractableTldr('<html><body><p>Trop court.</p></body></html>'),
    ).toBe(0);
  });
...
describe('detectVisibleSources', () => {
  it('returns 1 when ≥1 external <a> outside nav/footer', () => {
    const html =
      '<html><body><a href="https://www.constructeurs.fr/clio">source constructeur</a></body></html>';
    expect(detectVisibleSources(html, 'www.automecanik.com')).toBe(1);
  });
  it('returns 0 when all <a> are internal (same hostname)', () => {
    const html =
      '<a href="/pieces/freinage">interne</a><a href="https://www.automecanik.com/aide">aussi interne</a>';
    expect(detectVisibleSources(html, 'www.automecanik.com')).toBe(0);
  });
  it('returns 0 when external <a> are in <nav> or <footer>', () => {
    const html =
      '<nav><a href="https://twitter.com/x">tweet</a></nav><footer><a href="https://facebook.com">fb</a></footer>';
    expect(detectVisibleSources(html, 'www.automecanik.com')).toBe(0);
  });
});
