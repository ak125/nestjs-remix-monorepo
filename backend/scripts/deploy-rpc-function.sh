#!/bin/bash

# ⚡ Script de déploiement de la fonction RPC optimisée vers Supabase
# Objectif : passer de 138s à <5s en remplaçant 15+ requêtes HTTP par 1 RPC SQL

set -e

echo "🚀 Déploiement de la fonction RPC get_gamme_page_data_optimized..."

# Vérifier les variables d'environnement
if [ -z "$SUPABASE_DB_URL" ]; then
    echo "❌ Variable SUPABASE_DB_URL non définie"
    echo "📝 Ajoutez-la dans votre .env :"
    echo "   SUPABASE_DB_URL=\"postgresql://postgres:[PASSWORD]@db.[PROJECT_REF].supabase.co:5432/postgres\""
    exit 1
fi

# Chemin du fichier SQL
SQL_FILE="$(dirname "$0")/../prisma/supabase-functions/get_gamme_page_data_optimized.sql"

if [ ! -f "$SQL_FILE" ]; then
    echo "❌ Fichier SQL introuvable : $SQL_FILE"
    exit 1
fi

echo "📄 Fichier SQL : $SQL_FILE"
echo "🔗 Connexion à Supabase..."

# Option 1 : Via psql (si installé)
if command -v psql &> /dev/null; then
    echo "✅ psql détecté, déploiement en cours..."
    psql "$SUPABASE_DB_URL" -f "$SQL_FILE"
    echo "✅ Fonction RPC déployée avec succès via psql!"
else
    echo "⚠️  psql non installé"
    echo ""
    echo "📋 DÉPLOIEMENT MANUEL VIA SUPABASE DASHBOARD :"
    echo "   1. Aller sur https://supabase.com/dashboard/project/[PROJECT_ID]/sql/new"
    echo "   2. Copier le contenu de : $SQL_FILE"
    echo "   3. Exécuter la requête SQL"
    echo ""
    echo "💡 OU installer psql avec :"
    echo "   sudo apt-get install postgresql-client"
    exit 0
fi

echo ""
echo "✅ Déploiement terminé !"
echo ""
echo "🧪 TESTER LA FONCTION :"
echo "   curl \"http://localhost:3000/api/gamme-rest-optimized/2066/page-data-rpc-v2\""
echo ""
echo "📊 Temps attendu : <5s (vs 138s avant)"
