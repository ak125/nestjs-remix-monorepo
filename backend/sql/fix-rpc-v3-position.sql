-- ============================================================================
-- FIX: RPC V3 Position Detection - Ajoute detection depuis piece_name
-- ============================================================================
-- Probleme: Certaines pieces n'ont pas de position (Avant/Arriere)
-- Cause: rtp_psf_id = 9999 (Non specifie) n'existe pas dans pieces_side_filtre
--
-- Solution: Ajouter un 5e fallback qui detecte la position depuis piece_name
--
-- Ordre de detection (COALESCE):
--   1. pieces_relation_criteria (rcp_cri_value)
--   2. pieces_criteria (pc_cri_value)
--   3. pieces_side_filtre (psf_side via rtp_psf_id)
--   4. pieces.piece_name_side
--   5. NEW: Detection regex depuis pieces.piece_name
--
...
BEGIN
  ...
  -- PARTIE 1: SEO
  SELECT
    process_seo_template(COALESCE(sgc_h1, ''), p_type_id, p_pg_id, v_mf_id,
      v_marque_name, v_marque_alias, v_marque_id,
      v_modele_name, v_modele_alias, v_modele_id,
      v_type_name, v_type_alias, v_type_power_ps),
    process_seo_template(COALESCE(sgc_title, ''), p_type_id, p_pg_id, v_mf_id,
      v_marque_name, v_marque_alias, v_marque_id,
      v_modele_name, v_modele_alias, v_modele_id,
      v_type_name, v_type_alias, v_type_power_ps),
    process_seo_template(COALESCE(sgc_descrip, ''), p_type_id, p_pg_id, v_mf_id,
      v_marque_name, v_marque_alias, v_marque_id,
      v_modele_name, v_modele_alias, v_modele_id,
      v_type_name, v_type_alias, v_type_power_ps),
    process_seo_template(COALESCE(sgc_content, ''), p_type_id, p_pg_id, v_mf_id,
      v_marque_name, v_marque_alias, v_marque_id,
      v_modele_name, v_modele_alias, v_modele_id,
      v_type_name, v_type_alias, v_type_power_ps),
    process_seo_template(COALESCE(sgc_preview, ''), p_type_id, p_pg_id, v_mf_id,
      v_marque_name, v_marque_alias, v_marque_id,
      v_modele_name, v_modele_alias, v_modele_id,
      v_type_name, v_type_alias, v_type_power_ps)
  ...
  -- PARTIE 2-3: DONNEES PIECES
  WITH
  vehicle_info AS (
    SELECT
      at.type_id, at.type_name, at.type_alias, at.type_power_ps, at.type_power_kw,
      at.type_year_from, at.type_year_to, at.type_body, at.type_fuel, at.type_engine, at.type_liter,
      am.modele_id, am.modele_name, am.modele_alias, am.modele_pic,
      amarq.marque_id, amarq.marque_name, amarq.marque_alias, amarq.marque_logo
    FROM auto_type at
    JOIN auto_modele am ON am.modele_id = at.type_modele_id::INTEGER
    JOIN auto_marque amarq ON amarq.marque_id = am.modele_marque_id
    WHERE at.type_id::INTEGER = p_type_id AND at.type_display = '1'
    LIMIT 1
  ),
  ...
  gamme_info AS (
    SELECT pg.pg_id, pg.pg_name, pg.pg_alias, pg.pg_pic, COALESCE(cg.mc_mf_prime, '') as mf_id
    FROM pieces_gamme pg
    LEFT JOIN catalog_gamme cg ON cg.mc_pg_id::INTEGER = pg.pg_id
    WHERE pg.pg_id = p_pg_id
    LIMIT 1
  ...
  root_gamme AS (
    ...
    FROM pieces_gamme WHERE pg_id = p_pg_id LIMIT 1
  ),

  active_pieces AS (
    SELECT
      p.piece_id, p.piece_name, p.piece_ref, p.piece_ref_clean, p.piece_des,
      p.piece_has_img, p.piece_has_oem, p.piece_qty_sale, p.piece_qty_pack,
      p.piece_name_side, p.piece_name_comp, p.piece_fil_id, p.piece_fil_name, p.piece_pm_id,
      r.rtp_psf_id, r.rtp_pm_id,
      CASE
  ...
  best_prices AS (
    ...
    FROM pieces_price
    WHERE pri_piece_id IN (SELECT piece_id::TEXT FROM active_pieces) AND pri_dispo = '1'
    ORDER BY pri_piece_id, NULLIF(pri_type, '')::INTEGER DESC NULLS LAST
  ),

  first_images AS (
    SELECT DISTINCT ON (pmi_piece_id)
      pmi_piece_id as piece_id_text, pmi_folder, pmi_name,
      CASE WHEN pmi_name ~* '\.(webp|jpg|jpeg|png|gif)$' THEN pmi_name ELSE pmi_name || '.webp' END as pmi_name_with_ext
    FROM pieces_media_img
    WHERE pmi_piece_id IN (SELECT piece_id::TEXT FROM active_pieces) AND pmi_display = '1'
    ORDER BY pmi_piece_id, pmi_sort ASC
  ),

  all_images AS (
    SELECT pmi_piece_id as piece_id_text,
      jsonb_agg(jsonb_build_object(
        'url', v_cdn_base || '/rack-images/' || pmi_folder || '/' ||
          CASE WHEN pmi_name ~* '\.(webp|jpg|jpeg|png|gif)$' THEN pmi_name ELSE pmi_name || '.webp' END,
        'alt', pmi_name, 'sort', pmi_sort
      ) ORDER BY pmi_sort ASC) as images
    FROM pieces_media_img
    WHERE pmi_piece_id IN (SELECT piece_id::TEXT FROM active_pieces) AND pmi_display = '1'
  ...
  piece_brands AS (
    SELECT pm_id, pm_name, pm_logo, pm_nb_stars
    FROM pieces_marque
  ...
  criteria_positions AS (
    ...
    WHERE pc_piece_id::INTEGER IN (SELECT piece_id FROM active_pieces)
      AND pc_cri_id::TEXT = '100' AND pc_cri_value IS NOT NULL AND pc_cri_value != ''
  ...
  relation_criteria_positions AS (
    ...
    WHERE rcp_type_id::INTEGER = p_type_id AND rcp_pg_id::INTEGER = p_pg_id
  ...
  assembled_pieces AS (
    SELECT
      ...
      CASE
        ...
        ELSE 'Économique'
      END as qualite,
      -- FIX: 5 fallbacks pour la position (ajout detection depuis piece_name)
      COALESCE(
        ...
        -- 5. NEW: Detection depuis piece_name (dernier recours)
        CASE
  ...
  sorted_pieces AS (
    ...
    ORDER BY
      ...
      CASE filtre_side WHEN 'Avant' THEN 1 WHEN 'Arrière' THEN 2 WHEN 'Gauche' THEN 3 WHEN 'Droite' THEN 4 ELSE 5 END,
      prix_unitaire ASC NULLS LAST
  ),

  oem_brand AS (
    SELECT prb.prb_id FROM pieces_ref_brand prb
    INNER JOIN vehicle_info vi ON UPPER(prb.prb_name) = UPPER(vi.marque_name)
    LIMIT 1
  ),

  oem_refs_with_position AS (
    SELECT prs.prs_ref as ref,
      REGEXP_REPLACE(UPPER(REPLACE(REPLACE(REPLACE(prs.prs_ref, ' ', ''), '-', ''), '.', '')), '^[A-Z](?=[0-9])', '') as ref_normalized,
      sp.filtre_gamme, sp.filtre_side, sp.is_accessory,
      CASE WHEN sp.is_accessory THEN 100 WHEN sp.filtre_side = 'Avant' THEN 1 WHEN sp.filtre_side = 'Arrière' THEN 2
           WHEN sp.filtre_side = 'Gauche' THEN 3 WHEN sp.filtre_side = 'Droite' THEN 4 ELSE 10 END as group_priority,
      CASE WHEN prs.prs_ref ~ '^[A-Z0-9]{2,3} [0-9]{3} [0-9]{3} [A-Z0-9]{1,2}$' THEN 1
           WHEN prs.prs_ref ~ '^[A-Z0-9]{2,3} [0-9]{3} [0-9]{3}[A-Z0-9]{1,2}$' THEN 2
           WHEN prs.prs_ref ~ ' ' THEN 3 ELSE 10 END as format_score
  ...
  oem_refs_unique_global AS (
    SELECT DISTINCT ON (ref_normalized) ref, ref_normalized, filtre_gamme, filtre_side, is_accessory
    FROM oem_refs_with_position
    ORDER BY ref_normalized, group_priority ASC, format_score ASC, LENGTH(ref) DESC
  ),

  oem_refs_by_group AS (
    SELECT filtre_gamme, filtre_side, is_accessory,
      jsonb_agg(ref ORDER BY ref) as oem_refs, COUNT(*)::INTEGER as oem_refs_count
  ...
  grouped AS (
    SELECT
      sp.filtre_gamme as group_gamme, sp.filtre_side as group_side, sp.is_accessory,
      CASE WHEN sp.filtre_side IS NOT NULL AND sp.filtre_side != ''
        THEN sp.filtre_gamme || ' - ' || sp.filtre_side ELSE sp.filtre_gamme END as title_h2,
      jsonb_agg(jsonb_build_object(
      ...
      ) ORDER BY sp.prix_unitaire ASC NULLS LAST) as pieces,
  ...
  side_filter_agg AS (
    SELECT jsonb_build_object('id', 'side', 'name', 'Position', 'type', 'checkbox',
  ...
  quality_filter_agg AS (
    SELECT jsonb_build_object('id', 'quality', 'name', 'Qualité', 'type', 'checkbox',
      'options', COALESCE(jsonb_agg(DISTINCT jsonb_build_object(
  ...
  brand_filter_agg AS (
    SELECT jsonb_build_object('id', 'brand', 'name', 'Marque', 'type', 'checkbox',
      'options', COALESCE(jsonb_agg(DISTINCT jsonb_build_object(
  ...
  oem_refs_global AS (
    SELECT DISTINCT ON (ref_normalized) ref FROM oem_refs_unique_global
    ORDER BY ref_normalized, LENGTH(ref) DESC LIMIT 50
  )

  SELECT jsonb_build_object(
    'vehicle_info', (SELECT jsonb_build_object(
      'type_id', type_id, 'type_name', type_name, 'type_alias', type_alias,
      'type_power_ps', type_power_ps, 'type_power_kw', type_power_kw,
      'type_year_from', type_year_from, 'type_year_to', type_year_to,
      'type_body', type_body, 'type_fuel', type_fuel, 'type_engine', type_engine, 'type_liter', type_liter,
      'modele_id', modele_id, 'modele_name', modele_name, 'modele_alias', modele_alias, 'modele_pic', modele_pic,
      'marque_id', marque_id, 'marque_name', marque_name, 'marque_alias', marque_alias, 'marque_logo', marque_logo,
      'motor_codes', (SELECT codes FROM motor_codes)
    ) FROM vehicle_info),
    'gamme_info', (SELECT jsonb_build_object(
      'pg_id', pg_id, 'pg_name', pg_name, 'pg_alias', pg_alias, 'pg_pic', pg_pic, 'mf_id', mf_id
    ) FROM gamme_info),
    'seo', jsonb_build_object(
      'h1', COALESCE(v_seo_h1, ''), 'title', COALESCE(v_seo_title, ''),
      'description', COALESCE(v_seo_description, ''), 'content', COALESCE(v_seo_content, ''),
      'preview', COALESCE(v_seo_preview, '')
    ),
    'seo_templates', (SELECT jsonb_build_object(
      'h1', COALESCE(sgc_h1, ''), 'content', COALESCE(sgc_content, ''),
      'title', COALESCE(sgc_title, ''), 'description', COALESCE(sgc_descrip, ''), 'preview', COALESCE(sgc_preview, '')
    ) FROM __seo_gamme_car WHERE sgc_pg_id::INTEGER = p_pg_id LIMIT 1),
    ...
    'pieces', COALESCE((SELECT jsonb_agg(jsonb_build_object(
      ...
      'prix_unitaire', prix_unitaire, 'prix_ttc', prix_ttc, 'prix_consigne', prix_consigne,
      'quantite_vente', quantite_vente, 'dispo', dispo, 'image', image, 'images', images,
      'qualite', qualite, 'filtre_gamme', filtre_gamme, 'filtre_side', filtre_side,
      'has_image', has_image, 'has_oem', has_oem
    )) FROM sorted_pieces), '[]'::jsonb),
    'grouped_pieces', COALESCE((SELECT jsonb_agg(jsonb_build_object(
      'filtre_gamme', group_gamme, 'filtre_side', group_side, 'title_h2', title_h2,
      'pieces', pieces, 'oemRefs', oem_refs, 'oemRefsCount', oem_refs_count
    ) ORDER BY CASE WHEN is_accessory THEN 1 ELSE 0 END,
      CASE group_side WHEN 'Avant' THEN 1 WHEN 'Arrière' THEN 2 WHEN 'Gauche' THEN 3 WHEN 'Droite' THEN 4 ELSE 5 END
    ) FROM grouped), '[]'::jsonb),
    'blocs', COALESCE((SELECT jsonb_agg(jsonb_build_object(
      'filtre_gamme', group_gamme, 'filtre_side', group_side, 'title_h2', title_h2,
      'pieces', pieces, 'oemRefs', oem_refs, 'oemRefsCount', oem_refs_count
    ) ORDER BY CASE WHEN is_accessory THEN 1 ELSE 0 END,
      CASE group_side WHEN 'Avant' THEN 1 WHEN 'Arrière' THEN 2 WHEN 'Gauche' THEN 3 WHEN 'Droite' THEN 4 ELSE 5 END
    ) FROM grouped), '[]'::jsonb),
    'filters', jsonb_build_object('success', true, 'data', jsonb_build_object(
      'filters', COALESCE((SELECT jsonb_agg(f.filter_data) FROM (
        SELECT filter_data FROM side_filter_agg WHERE (filter_data->>'options')::jsonb != '[]'::jsonb
        UNION ALL SELECT filter_data FROM quality_filter_agg WHERE (filter_data->>'options')::jsonb != '[]'::jsonb
        UNION ALL SELECT filter_data FROM brand_filter_agg WHERE (filter_data->>'options')::jsonb != '[]'::jsonb
      ) f), '[]'::jsonb),
