...
export class ContentRefreshProcessor extends SupabaseBaseService {
  ...
  async handleContentRefresh(
    ...
    try {
      ...
      switch (pageType) {
        ...
        case 'R3_guide_howto': // @deprecated legacy — falls through to R6_guide_achat
        case 'R6_guide_achat': {
          // R6: Delegate to BuyingGuideEnricherService
  ...
  /** Safely serialize DB fields (TEXT[], JSONB, TEXT) to string */
  ...
  private async loadCurrentContent(
    ...
    if (
      ...
      const { data } = await this.client
        .from('__seo_gamme')
        .select('sg_content_draft')
        .eq('sg_pg_id', String(pgId))
        .single();
      return (data?.sg_content_draft as string) || '';
    }
    if (pageType === 'R3_conseils') {
      const { data } = await this.client
        .from('__seo_gamme_conseil')
  ...
  private buildSafeFallbackDraft(
    ...
    switch (pageType) {
      case 'R1_pieces':
        ...
        break;
      case 'R3_guide_howto': // @deprecated legacy
      case 'R6_guide_achat':
