 ...
 */
/* eslint-disable no-console -- CLI ops worker: the tab-separated REPORT to stdout IS its output */
...
async function main(): Promise<void> {
  ...
  if (!cfg)
    throw new Error(`no connectable supplier ${process.env.SUPPLIER_SPL}`);
  ...
  const explicit = (process.env.REFS ?? '')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
  let sel: Row[];
  if (explicit.length) {
    ...
    for (
      let i = 0;
      i < feed.length && m.size < N;
      i += Math.max(1, Math.floor(feed.length / (N * 0.3)))
    )
      m.set(feed[i].ref, feed[i]);
    sel = [...m.values()].slice(0, N);
  }
  console.log(
    `supplier=${cfg.supplierName} (${cfg.supplierId}) verify=${sel.length} byEan=${byEan}`,
  );
  ...
  console.log(
    'REPORT\tref\tfile_achat\tportal_achat\tstock\tecart_pct\tverdict',
  );
  let errors = 0;
  let done = 0;
  for (const r of sel) {
    ...
    if (achat == null) verdict = 'REVIEW';
      ...
      verdict =
        Math.abs(achat - r.achat) / achat <= 0.02 ? 'CONFIRMED' : 'FIX_FEED';
    const pct = achat ? (((r.achat - achat) / achat) * 100).toFixed(1) : '';
    console.log(
      `REPORT\t${r.ref}\t${r.achat}\t${achat ?? ''}\t${stock ?? ''}\t${pct}\t${verdict}`,
    );
