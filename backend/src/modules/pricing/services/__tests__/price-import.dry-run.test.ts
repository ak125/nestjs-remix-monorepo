...
describe('computeDryRun — UPDATE path', () => {
  ...
  it('weights CA impact by qty_sold_12m', () => {
    const r = computeDryRun(
      [line({ achatHtCents: 1300 })],
      new Map([['BP1', existing({ qtySold12m: 100 })]]),
      EMPTY_CATALOG,
    );
    expect(r.estimatedRevenueDeltaCents).toBe(
      (r.rows[0].newVenteTtcCents! - 2400) * 100,
    );
  });
  ...
  it('never touches MANUAL_OVERRIDE / FROZEN', () => {
    const r = computeDryRun(
      [line({ achatHtCents: 9999 })],
      new Map([['BP1', existing({ pricingState: 'FROZEN' })]]),
      EMPTY_CATALOG,
    );
  ...
  it('rejects a >30% jump via invariants', () => {
    const r = computeDryRun(
      [line()],
      new Map([['BP1', existing({ venteHtCents: 900 })]]),
      EMPTY_CATALOG,
    );
...
describe('computeDryRun — INSERT / recovery path', () => {
  const catalog = new Map<string, CatalogPiece>([
    ['VAL1', { priPieceIdI: 42 }],
  ]);
