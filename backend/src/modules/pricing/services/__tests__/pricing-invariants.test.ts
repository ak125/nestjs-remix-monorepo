...
describe('PricingInvariants — L2', () => {
  ...
  it.each<[string, PriceChainForValidation, string]>([
    [
      'negative price',
      { ...validChain, venteHtCents: -1 },
      'NON_FINITE_OR_NEGATIVE',
    ],
    [
      'non-finite',
      { ...validChain, venteTtcCents: NaN },
      'NON_FINITE_OR_NEGATIVE',
    ],
    ['achat ≤ 0', { ...validChain, achatHtCents: 0 }, 'ACHAT_NOT_POSITIVE'],
    [
      'vente < achat',
      { ...validChain, achatHtCents: 2000 },
      'VENTE_BELOW_ACHAT',
    ],
    [
      'marge ×10 absurd',
      { ...validChain, margePct: 1200 },
      'MARGE_EXCEEDS_MAX',
    ],
    [
      'tva not whitelisted',
      { ...validChain, tvaRate: 0.33 },
      'TVA_NOT_WHITELISTED',
    ],
  ...
  describe('delta guard', () => {
    ...
    it('passes a small delta', () => {
      expect(
        validatePriceChain({ ...validChain, currentVenteHtCents: 1850 }),
      ).toEqual([]);
    });
  });
  ...
  it('assertValidPriceChain throws PricingInvariantError carrying all violations', () => {
    try {
      ...
      expect(codes).toEqual(
        expect.arrayContaining(['ACHAT_NOT_POSITIVE', 'TVA_NOT_WHITELISTED']),
      );
    }
  });
});
