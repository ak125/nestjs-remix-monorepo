import { computeGridSimulation, type CostBucketAggregate } from '../pricing-simulation.core';
import type { PricingRule } from '../pricing-strategy.service';

const rule = (over: Partial<PricingRule>): PricingRule => ({
  id: 1, minCostCents: 0, maxCostCents: null, marginRate: 50, minMarginAmountCents: 0,
  maxMarginRate: null, customerType: 'B2C', supplierPmId: null, categoryGammeId: null,
  priority: 0, active: true, ...over,
});

describe('computeGridSimulation — read-only grid impact', () => {
  const buckets: CostBucketAggregate[] = [
    { representativeCostCents: 500, pieceCount: 100, sumAchatXQtyCents: 100000, sumVenteTtcXQtyCents: 198000 },
    { representativeCostCents: 40000, pieceCount: 10, sumAchatXQtyCents: 500000, sumVenteTtcXQtyCents: 750000 },
  ];
  ...
  it('applies the per-bucket rule and sums weighted revenue delta', () => {
    ...
    expect(r.buckets[0].estimatedRevenueCents).toBe(Math.round(Math.round(100000 * 1.65) * 1.2));
  ...
  it('flags buckets with no matching rule (left unchanged, no silent default)', () => {
    const r = computeGridSimulation(buckets, [rule({ minCostCents: 999999 })], 'B2C');
    expect(r.unmatchedBucketCount).toBe(2);
    expect(r.buckets[0].estimatedRevenueCents).toBe(r.buckets[0].currentRevenueCents);
  ...
  it('a more aggressive grid (lower rate) reduces estimated revenue on expensive bucket', () => {
    const r = computeGridSimulation(
      [buckets[1]],
      [rule({ id: 9, minCostCents: 30000, maxCostCents: null, marginRate: 10 })], // 25%→10%
      'B2C',
    );
