...
import { PopularProductsResponseDto, SuppliersResponseDto, CatalogHierarchyResponseDto } from './dtos';
...
export class CatalogController {
  ...
  async getPopularProducts() {
    try {
      const products = await this.catalogService.getPopularProducts();
      return {
        products,
        success: true,
      };
    ...
    /**
   * � Récupérer les équipementiers/marques
   */
  @Get('suppliers')
  @ApiOperation({ summary: 'Récupérer les équipementiers/marques' })
  @ApiResponse({
    status: 200,
    description: 'Liste des équipementiers récupérée avec succès',
  })
   ...
   */
  @Get('hierarchy')
  @ApiOperation({ summary: 'Récupérer la hiérarchie complète du catalogue' })
  @ApiResponse({
    status: 200,
    description: 'Hiérarchie du catalogue récupérée avec succès',
  })
  ...
  /**
   * GET /api/catalog/families/homepage
   * Récupérer les familles pour la page d'accueil (réplique logique PHP)
   */
  @Get('families/homepage')
  @ApiOperation({ 
    summary: 'Familles pour homepage (logique PHP)',
    description: 'Récupère les familles selon la logique exacte du PHP original'
  })
  @ApiResponse({ 
    status: 200, 
    description: 'Familles homepage récupérées avec succès'
  })
  async getHomepageFamilies() {
    this.logger.log('🏠 Requête familles homepage (logique PHP) reçue');
    return this.catalogService.getHomepageFamilies();
  }
