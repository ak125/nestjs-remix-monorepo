  ...
  async getPiecesV4Working(typeId: number, pgId: number) {
    ...
    try {
      ...
      const [piecesResult, marquesResult, pricesResult] = await Promise.all([
        this.client
          .from('pieces')
          .select('piece_id, piece_name, piece_ref, piece_des, piece_has_img, piece_qty_sale, piece_pm_id')
          .in('piece_id', pieceIds)
          .eq('piece_display', 1),

        pmIds.length > 0 ? this.client
          .from('pieces_marque')
          .select('pm_id, pm_name, pm_oes, pm_nb_stars')
          .in('pm_id', pmIds)
          .eq('pm_display', 1) : { data: [], error: null },
      ...
      const pieces = piecesData.map((piece) => {
        ...
        if (marqueEquip?.pm_oes === '1' || marqueEquip?.pm_oes === 'O') qualite = 'OES';
      ...
      // 5️⃣ PRIX MINIMUM
      const validPrices = pieces.map((p) => p.prix_ttc).filter((price) => price > 0);
      const minPrice = validPrices.length > 0 ? Math.min(...validPrices) : null;

      const duration = Date.now() - startTime;

      this.logger.log(`✅ [V4-WORKING] ${pieces.length} pièces en ${duration}ms`);
