...
export class FamilyGammeHierarchyService extends SupabaseBaseService {
    ...
    try {
      ...
      for (const gamme of gammes || []) {
        ...
        if (familyId && hierarchy[familyId]) {
          ...
          hierarchy[familyId].stats.manufacturers_count = uniqueManufacturers.size;
        }
      }
      ...
      this.logger.log(`✅ Hiérarchie construite: ${stats.total_families} familles, ${stats.total_gammes} gammes`);
  ...
  async getFamiliesWithSubcategories(): Promise<FamilyWithGammes[]> {
    try {
      ...
      this.logger.log(`✅ ${familiesWithGammes.length} familles avec sous-catégories préparées`);
      return familiesWithGammes;

    } catch (error) {
      this.logger.error('❌ Erreur familles avec sous-catégories:', error);
      throw new BadRequestException('Erreur lors de la récupération des familles avec sous-catégories');
    }
  }
   ...
   */
  private createManufacturerToFamilyMapping(families: CatalogFamily[]): { [manufacturerId: string]: string } {
    ...
    for (const family of families) {
      ...
      for (let mfId = startManufacturer; mfId <= endManufacturer && mfId <= 20; mfId++) {
   ...
   */
  private mapGammeToFamily(gamme: CatalogGamme, mapping: { [manufacturerId: string]: string }): string | null {
   ...
   */
  private calculateHierarchyStats(hierarchy: FamilyGammeHierarchy, allGammes: CatalogGamme[]): HierarchyStats {
    const familiesWithGammes = Object.values(hierarchy).filter(item => item.stats.total_gammes > 0);
  ...
  async getFamilyWithGammesById(familyId: string): Promise<FamilyWithGammes | null> {
    try {
      ...
      this.logger.log(`✅ Famille ${familyId} avec ${familyWithGammes.gammes_count} gammes récupérée`);
