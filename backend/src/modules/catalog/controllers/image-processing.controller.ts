...
export class ImageProcessingController {
  ...
  async analyzeImage(@Param('imageName') imageName: string) {
    ...
    try {
      ...
      this.logger.log(
        `✅ Image ${imageName} analysée: ${analysis.width}x${analysis.height}`,
      );
      return {
        success: true,
        data: analysis,
  ...
  async generateVariants(@Param('imageName') imageName: string) {
    ...
    try {
      const imagePath = `articles/familles-produits/${imageName}.webp`;
      const result =
        await this.imageService.generateFamilyImageVariants(imagePath);

      this.logger.log(
        `✅ Variantes générées pour ${imageName}: ${Object.keys(result.variants).length} variantes`,
      );
      return {
        success: true,
        data: result,
  ...
  async processAllImages() {
    ...
    try {
      ...
      this.logger.log(
        `✅ Traitement global terminé: ${result.processed} images`,
      );
      return {
        success: true,
        data: result,
  ...
  async resizeImage(
    ...
    try {
      ...
      const originalBuffer =
        await this.imageService.downloadImageFromSupabase(imagePath);
      ...
      const url = await this.imageService.uploadImageToSupabase(
        resizedBuffer,
        resizedPath,
      );

      this.logger.log(
        `✅ Image ${imageName} redimensionnée: ${resizeOptions.width}x${resizeOptions.height}`,
      );
      return {
        success: true,
        data: {
          ...
          compressionRatio: Math.round(
            (1 - resizedBuffer.length / originalBuffer.length) * 100,
          ),
        },
