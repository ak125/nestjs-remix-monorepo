...
export class GammeController {
  ...
  @ApiOperation({
    ...
    description:
      'Retourne les gammes featured, populaires et statistiques optimisées pour la homepage',
  })
  @ApiQuery({
  ...
  async getHomepageData(
    @Query('includeFeatured', new ParseBoolPipe({ optional: true }))
    includeFeatured?: boolean,
    @Query('includeStats', new ParseBoolPipe({ optional: true }))
    includeStats?: boolean,
    @Query('maxCategories', new ParseIntPipe({ optional: true }))
    maxCategories?: number,
  ) {
    try {
      this.logger.log(
        `🏠 Requête homepage gammes - Featured: ${includeFeatured}, Stats: ${includeStats}, Max: ${maxCategories}`,
      );
      ...
      this.logger.log(
        `✅ Homepage gammes: ${result.featured_gammes.length} featured, ${result.popular_gammes.length} populaires`,
      );
      return result;
    } catch (error) {
  ...
  @ApiResponse({
    status: 200,
    description: 'Gammes featured récupérées avec succès',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Retourne les gammes les plus commandées (basé sur les statistiques de vente)',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: 'Gammes populaires récupérées avec succès',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Retourne toutes les gammes actives avec possibilité de filtrage',
  })
  @ApiQuery({
    name: 'featured',
    required: false,
  ...
  async getAllGammes(
    ...
    try {
      this.logger.log(
        `📋 Requête toutes gammes - Featured: ${featured}, Brand: ${brandId}, Filter: ${categoryFilter}`,
      );
  ...
  @ApiOperation({
    ...
    description:
      "Retourne les détails complets d'une gamme avec produits populaires optionnels",
  })
  @ApiParam({
    name: 'id',
    type: Number,
  ...
  @ApiResponse({
    status: 200,
    description: 'Détails de la gamme récupérés avec succès',
  })
  ...
  async getGammeById(
    ...
    try {
      this.logger.log(
        `🔍 Requête détails gamme ID: ${gammeId} - Produits: ${includeProducts}`,
      );

      const result = await this.gammeService.getGammeById(
        gammeId,
        includeProducts,
      );
  ...
  @ApiOperation({
    ...
    description:
      "Retourne les détails d'une gamme avec la liste de ses pièces détachées associées",
  })
  @ApiParam({
    name: 'code',
  ...
  async getGammeWithPieces(@Param('code') gammeCode: string) {
    try {
      ...
      this.logger.log(
        `✅ API - Gamme ${gammeCode} avec ${gamme.pieces?.length || 0} pièces`,
      );
      return {
        success: true,
        data: gamme,
  ...
  @ApiOperation({
    ...
    description:
      'Retourne la hiérarchie complète des gammes avec leurs sous-catégories',
  })
  @ApiResponse({
    status: 200,
  ...
  async getGammeHierarchy() {
    try {
      ...
      this.logger.log(
        `✅ API - Hiérarchie: ${hierarchy.length} gammes principales`,
      );
      return {
        success: true,
        data: hierarchy,
        metadata: {
          ...
          total_subcategories: hierarchy.reduce(
  ...
  @ApiOperation({
    ...
    description:
      'Retourne title, description, mots-clés et breadcrumbs pour le SEO',
  })
  @ApiParam({
    name: 'code',
    description: 'Code (alias) de la gamme',
  ...
  async getGammeMetadata(@Param('code') gammeCode: string) {
    try {
  ...
  @ApiOperation({
    ...
    description:
      'Recherche textuelle dans les noms, descriptions et codes de gammes',
  })
  @ApiQuery({
    name: 'q',
  ...
  async searchGammes(
    ...
    @Query('includeProducts', new ParseBoolPipe({ optional: true }))
    includeProducts?: boolean,
    @Query('onlyFeatured', new ParseBoolPipe({ optional: true }))
    ...
    try {
      ...
      this.logger.log(
        `✅ API - Recherche "${query}": ${results.length} résultats`,
      );
      return {
        success: true,
        data: results,
