...
export class UsersController {
  ...
  async getAllUsers(
    ...
    try {
      console.log(`🔍 GET /api/users - page: ${page}, limit: ${limit}, search: ${search}`);
  ...
  async getUserByEmail(@Param('email') email: string): Promise<UserResponseDto> {
    try {
      ...
      if (!user) {
        throw new HttpException('Utilisateur non trouvé avec cet email', HttpStatus.NOT_FOUND);
      }
      ...
      const errorMessage = error instanceof Error ? error.message : 'Erreur lors de la récupération de l\'utilisateur par email';
      const errorStatus = error instanceof HttpException ? error.getStatus() : HttpStatus.INTERNAL_SERVER_ERROR;
  ...
  async changePassword(
    ...
    try {
      ...
      const success = await this.usersService.changePassword(id, changePasswordDto);
  ...
  async updateUserLevel(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Erreur lors de la mise à jour du niveau';
      const errorStatus = error instanceof HttpException ? error.getStatus() : HttpStatus.INTERNAL_SERVER_ERROR;
  ...
  @Get('level/:level')
  async getUsersByLevel(@Param('level') level: string): Promise<UserResponseDto[]> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Erreur lors de la récupération des utilisateurs par niveau';
      const errorStatus = error instanceof HttpException ? error.getStatus() : HttpStatus.INTERNAL_SERVER_ERROR;
  ...
  async getActiveUsers(
    ...
    try {
      ...
      console.log(`✅ GET /api/users/active - page: ${pageNum}, limit: ${limitNum}`);
      ...
      const errorMessage = error instanceof Error ? error.message : 'Erreur lors de la récupération des utilisateurs actifs';
      const errorStatus = error instanceof HttpException ? error.getStatus() : HttpStatus.INTERNAL_SERVER_ERROR;
