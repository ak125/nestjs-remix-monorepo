...
export class AdminSeoController {
  ...
  async getDashboard() {
    ...
    const failedChecks = recentJobs.filter(j => j.returnvalue?.errorCount > 0).length;
    const warningChecks = recentJobs.filter(j => j.returnvalue?.warningCount > 0).length;
    ...
    return {
      ...
      data: {
        ...
        stats: {
          ...
          avgErrorCount: timeline.reduce((sum, t) => sum + t.errorCount, 0) / timeline.length,
          avgWarningCount: timeline.reduce((sum, t) => sum + t.warningCount, 0) / timeline.length,
        },
      },
    };
  }
  ...
  async getUrlsAtRisk() {
    ...
    for (const job of jobs) {
      ...
      for (const alert of alerts) {
        if (alert.status === 'error' && alert.piecesCount === 0) {
          ...
          if (!existing || new Date(alert.checkedAt) > new Date(existing.lastSeen)) {
    ...
    const urlsAtRisk = Array.from(urlsWithErrors.values())
