...
export class AdminSuppliersController {
  ...
  async getAllSuppliers(@Query() query: SupplierQuery, @Request() req: any) {
    try {
      ...
      const result = await this.suppliersService.getAllSuppliers(query, currentUserId);
  ...
  async getSupplierStats() {
    try {
      ...
      this.logger.error('Erreur lors de la récupération des stats fournisseurs:', error);
      return {
        success: false,
  ...
  async createSupplier(@Body() data: CreateSupplier, @Request() req: any) {
    try {
      ...
      const supplier = await this.suppliersService.createSupplier(data, currentUserId);
  ...
  async updateSupplier(
    ...
    try {
      ...
      const supplier = await this.suppliersService.updateSupplier(updateData, currentUserId);
      ...
      this.logger.error(`Erreur lors de la mise à jour du fournisseur ${id}:`, error);
      return {
        success: false,
  ...
  async toggleSupplierStatus(
    ...
    try {
      ...
      const supplier = await this.suppliersService.toggleSupplierStatus(id, isActive, currentUserId);
      ...
      this.logger.error(`Erreur lors du changement de statut du fournisseur ${id}:`, error);
      return {
        success: false,
