...
export class ProductsEnhancementService {
   ...
   */
  async validateProductBusinessRules(createProductDto: CreateProductDto): Promise<string[]> {
    const errors: string[] = [];
    ...
    try {
      ...
      if (existingProduct) {
        errors.push(`Un produit avec le SKU '${createProductDto.sku}' existe déjà`);
      }

      // Vérifier que la gamme existe
      if (createProductDto.range_id) {
        const rangeExists = await this.checkRangeExists(createProductDto.range_id);
        if (!rangeExists) {
          errors.push(`La gamme avec l'ID ${createProductDto.range_id} n'existe pas`);
        }
      }

      // Vérifier que la marque existe
      if (createProductDto.brand_id) {
        const brandExists = await this.checkBrandExists(createProductDto.brand_id);
        if (!brandExists) {
          errors.push(`La marque avec l'ID ${createProductDto.brand_id} n'existe pas`);
        }
      }

      // Validation du prix vs stock
      if (createProductDto.base_price && createProductDto.base_price > 10000) {
        if (!createProductDto.stock_quantity || createProductDto.stock_quantity > 100) {
          errors.push('Les produits de plus de 10000€ ne peuvent avoir plus de 100 unités en stock');
        }
      }

      return errors;
    } catch (error) {
      this.logger.error('Erreur lors de la validation des règles métier:', error);
  ...
  private async checkRangeExists(rangeId: number): Promise<boolean> {
    try {
      ...
      this.logger.error(`Erreur lors de la vérification de la gamme ${rangeId}:`, error);
      return false;
    }
  }
  ...
  private async checkBrandExists(brandId: number): Promise<boolean> {
    try {
      ...
      this.logger.error(`Erreur lors de la vérification de la marque ${brandId}:`, error);
      return false;
    }
  }
    ...
    try {
      ...
      if (product.base_price) {
        if (product.base_price > 1000) {
          ...
          recommendations.reasoning.push('Produit de haute valeur: stock réduit recommandé');
          ...
          recommendations.reasoning.push('Produit de faible valeur: stock élevé recommandé');
        }
      }

      // Ajustements basés sur la gamme
      if (product.range_id) {
        recommendations.reasoning.push(`Ajustement basé sur la gamme ${product.range_id}`);
      }

      return recommendations;
    } catch (error) {
      this.logger.error(`Erreur lors du calcul des recommandations pour ${productId}:`, error);
      recommendations.reasoning.push('Erreur lors du calcul: utilisation des valeurs par défaut');
      return recommendations;
    }
  }
  ...
  async analyzeSearchTrends(filters: SearchProductDto): Promise<{
    ...
    try {
      ...
      if (filters.search) {
        analysis.popular_filters['search'] = (analysis.popular_filters['search'] || 0) + 1;
        
        // Suggestions basées sur la recherche
        const searchTerms = filters.search.toLowerCase().split(' ');
        if (searchTerms.includes('frein')) {
          analysis.suggested_searches.push('plaquettes de frein', 'disques de frein', 'étriers de frein');
        }
        if (searchTerms.includes('moteur')) {
          analysis.suggested_searches.push('huile moteur', 'filtre moteur', 'courroie distribution');
        }
      }

      if (filters.brandId) {
        analysis.popular_filters['brandId'] = (analysis.popular_filters['brandId'] || 0) + 1;
      }

      if (filters.rangeId) {
        analysis.popular_filters['rangeId'] = (analysis.popular_filters['rangeId'] || 0) + 1;
      }

      // Optimisations suggérées
      if (filters.page && filters.page > 10) {
        analysis.optimization_hints.push('Affiner la recherche pour réduire le nombre de pages');
      }

      if (!filters.brandId && !filters.rangeId && !filters.search) {
        analysis.optimization_hints.push('Utiliser des filtres pour améliorer la pertinence');
      }
  ...
  async generateDataQualityReport(): Promise<{
    ...
    try {
      ...
      const completeness = (report.total_products - report.missing_descriptions - report.missing_prices) / report.total_products;
      const uniqueness = (report.total_products - report.duplicate_skus) / report.total_products;
      report.quality_score = Math.round((completeness * 0.7 + uniqueness * 0.3) * 100);

      // Générer des recommandations
      if (report.missing_descriptions > 0) {
        report.recommendations.push(`Ajouter des descriptions à ${report.missing_descriptions} produits`);
      }
      if (report.missing_prices > 0) {
        report.recommendations.push(`Définir les prix pour ${report.missing_prices} produits`);
      }
      if (report.duplicate_skus > 0) {
        report.recommendations.push(`Résoudre ${report.duplicate_skus} doublons de SKU`);
      }
      if (report.quality_score < 80) {
        report.recommendations.push('Améliorer la qualité globale des données produit');
      }

      return report;
    } catch (error) {
      this.logger.error('Erreur lors de la génération du rapport qualité:', error);
      throw error;
    }
  }
}
