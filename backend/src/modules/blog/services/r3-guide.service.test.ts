...
describe('R3GuideService — cache + single-flight + invalidation (PR-A)', () => {
  describe('cache hit / miss', () => {
    ...
    it('returns null + skips cache write when no article matches the gamme', async () => {
      ...
      expect(await cache.get('r3-guide:v4:inconnu')).toBeNull();
    });
  });
...
describe('R3GuideService — sections META servies seulement si conformes au contrat « liens »', () => {
  ...
  it('payload filtre mis en cache sous la cle v4 (META filtre et ancre diagnostic stable)', async () => {
    ...
    const cached = await cache.get<{ metaSections: unknown[] }>(
      `r3-guide:v4:${PG_ALIAS}`,
    );
    expect(cached?.metaSections).toEqual([]);
    expect(await cache.get(`r3-guide:v2:${PG_ALIAS}`)).toBeNull();
    expect(await cache.get(`r3-guide:v3:${PG_ALIAS}`)).toBeNull();
  });
});

describe('R3 S2_DIAG — destination des redirections R5 (ADR-027)', () => {
  it.each([
    'Diagnostic rapide du disque de frein',
    'Symptomes et controles utiles',
  ])(
    'conserve une ancre stable malgre le titre %s, ainsi que son ancien fragment',
    async (title) => {
      const { service, seoService } = await buildService();
      seoService.getGammeConseil.mockResolvedValue([
        {
          title,
          content:
            '<table><tr><td>Signal</td><td>Hypothese</td><td>Controle</td></tr></table>',
          sectionType: 'S2_DIAG',
          order: 7,
          qualityScore: null,
          sources: [],
      ...
      const result = await service.getR3GuidePayload('filtre-a-huile');
      const section = result?.bodySections.find(
        (s) => s.sectionType === 'S2_DIAG',
      );
      expect(section?.anchor).toBe('diagnostic-rapide');
      expect(section?.legacyAnchor).toBeDefined();
      expect(section?.legacyAnchor).not.toBe(section?.anchor);
