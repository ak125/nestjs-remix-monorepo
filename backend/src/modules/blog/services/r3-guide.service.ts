 ...
 *  v4 (2026-09-13) : ancre contractuelle S2_DIAG pour les redirections R5. */
const R3_CACHE_PREFIX = 'r3-guide:v4:';
...
export class R3GuideService {
  ...
  private async mapConseilSection(
    ...
    const titleAnchor = slugifyTitle(s.title);
    const anchor =
      s.sectionType === 'S2_DIAG' ? 'diagnostic-rapide' : titleAnchor;
    return {
      sectionType: s.sectionType,
      title: s.title,
      anchor,
      ...(anchor !== titleAnchor ? { legacyAnchor: titleAnchor } : {}),
      order: s.order ?? index,
      html,
      sources: s.sources,
