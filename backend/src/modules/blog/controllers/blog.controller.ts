...
export class BlogController {
  ...
  async searchBlog(
    ...
    try {
      ...
      const results = await this.blogService.searchBlog(
        query,
        type || 'all',
        page,
        limit,
      ...
      this.logger.log(
        `🔍 Recherche blog: "${query}" - ${results.results?.length || 0} résultats`,
      );
  ...
  private async getRelatedArticles(
    ...
    try {
      ...
      const searchResults = await this.blogService.searchBlog(
        searchQuery,
        'all',
        1,
        limit + 1,
