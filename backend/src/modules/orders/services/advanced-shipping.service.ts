...
export class AdvancedShippingService {
  ...
  async calculateShippingFee(
    ...
    try {
      ...
      const deliveryZone = await this.determineDeliveryZone(
        validatedParams.zipCodeDeliveryId,
      );
      ...
      throw new BadRequestException(
        `Erreur de calcul de livraison: ${errorMessage}`,
      );
    }
  }
   ...
   */
  private calculateWeightBasedShipping(
    weight: number,
    zone: DeliveryZone,
  ): {
    baseRate: number;
  ...
  private applyBusinessRules(
    ...
    params: ShippingCalculationParams,
    zone: DeliveryZone, // eslint-disable-line @typescript-eslint/no-unused-vars
  ): {
    amount: number;
    discountApplied: number;
  } {
   ...
   */
  async calculateLegacyShippingFee(
    params: ShippingCalculationParams,
  ): Promise<number> {
  ...
  async getCarrierInfo(zoneId: string): Promise<{
    ...
    return (
      carriers[zoneId as keyof typeof carriers] || carriers['INTERNATIONAL']
    );
  }
