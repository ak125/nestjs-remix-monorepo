...
export class TaxCalculationService {
  ...
  async calculateOrderTax(
    ...
    const subtotalTTC = itemsDetail.reduce(
      (sum, item) => sum + item.totalTTC,
      0,
    );
    const subtotalTaxAmount = itemsDetail.reduce(
    ...
    const totalTaxAmount =
      subtotalTaxAmount + shippingTaxAmount - discountTaxAmount;
  ...
  async getTaxConfigFromLegacyTables(): Promise<Partial<TaxConfig>> {
    try {
      ...
      this.logger.warn(
        'Impossible de récupérer la config taxes depuis les tables legacy',
        error,
      );
      return {};
    }
  ...
  async getTaxConfigForClient(
    ...
    try {
      ...
      const isProClient =
        clientInfo?.cst_is_pro === '1' || clientType === 'professionnel';
  ...
  validateTaxCalculation(result: OrderTaxResult): {
    ...
    if (difference > 0.01) {
      errors.push(
        `Incohérence HT + TVA ≠ TTC (écart: ${difference.toFixed(2)}€)`,
      );
    }
    ...
    const itemsTotal = result.itemsDetail.reduce(
      (sum, item) => sum + item.totalTTC,
      0,
    );
    ...
    if (totalDifference > 0.01) {
      errors.push(
        `Incohérence dans le calcul des totaux (écart: ${totalDifference.toFixed(2)}€)`,
      );
    }
  ...
  getTaxSummary(result: OrderTaxResult): {
    ...
    result.itemsDetail.forEach((item) => {
      const existing = taxByRate.get(item.taxRate) || {
        baseHT: 0,
        taxAmount: 0,
      };
