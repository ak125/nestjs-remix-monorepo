...
export interface SearchParams {
  ...
  filters?: {
    brandId?: number;
    brand?: string; // 🏭 Nouveau : recherche par nom de marque/équipementier
    categoryId?: number;
    priceMin?: number;
    priceMax?: number;
    inStock?: boolean;
    equipment?: string; // 🏭 Nouveau : alias pour équipementier
    compatibility?: {
      make?: string;
      model?: string;
      year?: number;
  ...
  sort?: {
    field: 'relevance' | 'price' | 'name' | 'date' | 'brand'; // 🏭 Ajout du tri par marque
    order: 'asc' | 'desc';
  };
  pagination?: {
    page: number;
  ...
  options?: {
    ...
    fuzzySearch?: boolean;
    synonyms?: boolean;
    includeBrands?: boolean; // 🏭 Nouveau : inclure les infos marques
  };
}
...
export class SearchService {
  ...
  private async searchV8Enhanced(params: SearchParams): Promise<SearchResult> {
    ...
    const searchOptions = {
      ...
      facets: [
        'marque', // 🔧 Utilise les vrais noms d'attributs de l'index véhicules
        'modele',
        'carburant', // au lieu de fuel_type
        'transmissionType', // au lieu de transmission
        'anneeDebut', // au lieu de year
        'anneeFin',
        'puissanceCV',
        'cylindree',
      ],
    ...
    this.logger.debug(`🔍 Recherche V8 pour: "${params.query}"`);
    ...
    this.logger.debug(`🚗 Résultats véhicules:`, vehicleResults);
    this.logger.debug(`🔧 Résultats produits:`, productResults);
  ...
  private normalizeParams(params: SearchParams): SearchParams {
    ...
    if (brandDetection.detected) {
      // Enrichir les filtres avec la marque détectée
      normalized.filters = {
        ...normalized.filters,
    ...
    return {
      ...
      options: {
        ...
        facets: false,
        suggestions: false,
        includeBrands: true, // 🏭 Activer par défaut
        ...params.options,
      },
    };
  }
  ...
  /**
   * 🏭 Détection de marques/équipementiers dans la requête
   */
  private detectBrandInQuery(query: string): {
    detected: boolean;
    brand?: string;
    cleanedQuery: string;
  } {
    if (!query) return { detected: false, cleanedQuery: query };
    ...
    return { detected: false, cleanedQuery: query };
  }
  ...
  private mergeResultsIntelligent(
    ...
    this.logger.debug('🔄 Items fusionnés:', items.length);
  ...
  private buildFiltersV8Enhanced(filters?: SearchParams['filters']): string[] {
    ...
    // 🏭 Nouveau : filtrage par nom de marque/équipementier
    if (filters.brand) {
      filterQueries.push(`brand = "${filters.brand}"`);
    }

    // 🏭 Alias pour équipementier
    if (filters.equipment) {
      filterQueries.push(`brand = "${filters.equipment}"`);
    }
