 ...
 * ✅ Cache intelligent par profil utilisateur
 * ✅ Suggestions basées sur l'historique
 * ✅ Comparaison de prix marché
 */
@Injectable()
export class VehicleSearchService {
  ...
  async searchVehicles(
    ...
    if (cached) {
      this.logger.log(`⚡ Cache hit véhicules: "${query.query}"`);
      return cached;
    }

    try {
      const searchBody = this.buildElasticsearchQuery(query);
      this.logger.log(`🔍 Recherche ES véhicules: "${query.query}"`);
  ...
  private buildElasticsearchQuery(
    ...
    return {
      ...
      sort: [
        'createdAt:desc', // 🔧 Remplace _score par createdAt:desc
        'updatedAt:desc',
      ],
      aggs: {
  ...
  /**
   * 🔄 Transformation des résultats ES en format API
   */
  ...
  /**
   * 🏷️ Extraction des facettes pour filtrage
   */
  ...
  private async generateSuggestions(
    ...
    try {
      ...
      this.logger.warn('Erreur génération suggestions:', error.message);
      return [];
    }
  }

  /**
   * ⏱️ Calcul du TTL de cache adaptatif
   */
  ...
  /**
   * 📊 Nombre total de véhicules indexés
   */
  async getTotalCount(): Promise<number> {
    try {
      ...
      this.logger.error('Erreur comptage véhicules:', error);
      return 0;
    }
  }
  ...
  private getPriceRangeLabel(from?: number, to?: number): string {
    if (!from && to) return `Moins de ${to.toLocaleString()}€`;
    if (from && !to) return `Plus de ${from.toLocaleString()}€`;
