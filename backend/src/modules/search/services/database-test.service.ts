...
export class DatabaseTestService {
  ...
  async testRpcApproach() {
    try {
      ...
      this.logger.log(`✅ RPC Approach: ${data?.length} vehicles in ${duration}ms`);
      return { 
        success: true, 
        approach: 'RPC',
        count: data?.length || 0,
  ...
  async testApplicationJoinApproach() {
    try {
      ...
      const { data: types, error: typesError } = await client
        .from('auto_type')
        .select('type_id, type_name, type_modele_id, type_marque_id, type_fuel, type_power_ps')
        .eq('type_display', '1')
        .limit(10);
      ...
      const modeleIds = [...new Set(types?.map(t => parseInt(t.type_modele_id)).filter(Boolean))];
      ...
      this.logger.log(`✅ App Join Approach: ${vehiclesWithRelations?.length} vehicles in ${duration}ms`);
      return { 
        success: true, 
  ...
  private getRecommendation(successfulApproaches: any[]) {
    ...
    const hasAppJoin = successfulApproaches.some(a => a.approach === 'APPLICATION_JOIN');
