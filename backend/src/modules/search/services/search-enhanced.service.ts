...
export class SearchService {
   ...
   * Support complet API V7/V8 avec optimisations Enterprise
   */
  ...
  private async routeSearch(params: SearchParams): Promise<SearchResult> {
    switch (params.type) {
      ...
      default:
        ...
        return this.searchV8Advanced(params);
    }
  }

  /**
   * 🔍 Recherche V7 - Mode compatibilité optimisé
   */
  ...
  private async searchV8(params: SearchParams): Promise<SearchResult> {
    ...
    const [vehicleResults, productResults] = await Promise.allSettled([
      ...
      this.meilisearch.searchProducts(params.query, searchOptions),
    ]);

    // Fusion intelligente des résultats
    const items = this.mergeResults(vehicleResults, productResults, params);
  ...
  /**
   * 🚀 Recherche V8 Avancée - Mode Enterprise avec IA
   */
  private async searchV8Advanced(params: SearchParams): Promise<SearchResult> {
    ...
    // Recherche hybride avec scoring IA
    const promises = [
      this.meilisearch.searchVehicles(params.query, {
        ...baseOptions,
    ...
    // Si synonymes activés, ajouter recherche étendue
    if (params.options?.synonyms) {
      const expandedQuery = await this.expandQueryWithSynonyms(params.query);
    ...
    const items = this.mergeAdvancedResults(results, params);
    const totalHits = this.calculateAdvancedTotalHits(results);
  ...
  /**
   * 📋 Recherche par référence optimisée
   */
  ...
  private mergeAdvancedResults(results: any[], params: SearchParams): any[] {
    const allItems: any[] = [];
    ...
    for (const result of results) {
      if (result.status === 'fulfilled' && result.value.hits) {
        for (const hit of result.value.hits) {
          const uniqueId = `${hit.source || 'unknown'}_${hit.id}`;
    ...
    // Tri avancé avec boost pour correspondances exactes
    return allItems
      .sort((a, b) => {
  ...
  private async expandQueryWithSynonyms(query: string): Promise<string> {
    try {
      // Logique d'expansion avec synonymes (à implémenter selon besoins)
      const synonyms = await this.meilisearch.getSynonyms(query);
      return synonyms.length > 0 ? `${query} ${synonyms.join(' ')}` : query;
    } catch {
      return query;
    }
  }
  ...
  private buildFiltersV8Advanced(filters?: SearchParams['filters']): string[] {
    const filterQueries = this.buildFiltersV8(filters);

    // Filtres avancés additionnels
    if (filters) {
      // Filtres géographiques (à implémenter si nécessaire)
      // Filtres temporels
      // Filtres de qualité/rating
    }
  ...
  private async extractFacetsFromResults(
    ...
    facets.sources = Object.entries(sources).map(([source, items]) => ({
