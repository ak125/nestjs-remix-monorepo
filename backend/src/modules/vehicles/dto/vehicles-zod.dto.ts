...
export const VehicleSearchSchema = z.object({
  ...
  fuelType: z.enum([
    ...
    'GPL', 'GNV', 'Essence-Électrique', 'Extension d\'aut'
  ]).optional(),
});
...
export const VehicleFilterSchema = z.object({
  search: z.string().min(1, 'Le terme de recherche doit contenir au moins 1 caractère').optional(),
  brandId: z.number().int().positive('L\'ID marque doit être positif').optional(),
  modelId: z.number().int().positive('L\'ID modèle doit être positif').optional(),
...
export const TypeQuerySchema = z.object({
  ...
  fuelType: z.enum([
    'Essence', 'Diesel', 'Électrique', 'Hybride', 
    'GPL', 'GNV', 'Essence-Électrique'
  ]).optional(),
  minPower: z.string().regex(/^\d+$/, 'Puissance doit être un nombre').optional(),
  maxPower: z.string().regex(/^\d+$/, 'Puissance doit être un nombre').optional(),
}).transform((data) => ({
  search: data.search,
  limit: data.limit ? parseInt(data.limit) : 50,
  page: data.page ? parseInt(data.page) : 0,
 ...
 */
export const AdvancedVehicleSearchSchema = z.object({
  // Recherche textuelle
  search: z.string().min(1, 'Le terme de recherche doit contenir au moins 1 caractère').optional(),
  ...
  yearTo: z.number().int().min(1900).max(new Date().getFullYear() + 5).optional(),
  onlyActive: z.boolean().default(true),
  ...
  if (data.minPowerPs && data.maxPowerPs && data.minPowerPs > data.maxPowerPs) {
    return false;
  }
  if (data.minPowerKw && data.maxPowerKw && data.minPowerKw > data.maxPowerKw) {
    return false;
  }
...
export type AdvancedVehicleSearchDto = z.infer<typeof AdvancedVehicleSearchSchema>;
