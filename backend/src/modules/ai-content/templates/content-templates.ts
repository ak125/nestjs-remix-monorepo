...
Tu respectes les consignes et le ton demandé tout en restant créatif et engageant.`,
    user: (ctx) => {
      ...
      let userPrompt = prompt || 'Crée du contenu pertinent selon le contexte fourni.';
...
Mets en valeur les bénéfices pour le client, pas seulement les caractéristiques techniques.`,
    user: (ctx) => {
      const { productName, category, features, specifications, targetAudience, tone, length } = ctx;
...
Tu crées des textes persuasifs qui convertissent, en utilisant les principes de copywriting éprouvés.`,
    user: (ctx) => {
      const { objective, targetAudience, product, benefits, tone, length } = ctx;
...
Tu connais les codes de chaque plateforme et sais créer des posts qui génèrent de l'interaction.`,
    user: (ctx) => {
      const { platform, message, callToAction, hashtags, tone, includeEmojis } = ctx;
...
Tu sais structurer un email pour maximiser les taux d'ouverture et de clic.`,
    user: (ctx) => {
      const { subject, goal, targetAudience, tone, includeSubject, offer } = ctx;
...
export const TONE_MODIFIERS: Record<Tone, string> = {
  ...
  casual: 'Utilise un langage décontracté et accessible, comme si tu parlais à un ami.',
  friendly: 'Sois chaleureux, bienveillant et proche de ton audience.',
  technical: 'Utilise un vocabulaire technique précis, adapté à des experts du domaine.',
  persuasive: 'Sois convaincant et utilise des techniques de persuasion pour inciter à l\'action.',
  informative: 'Concentre-toi sur la transmission d\'informations claires et objectives.',
};
