...
export class OrdersService {
  ...
  async findAll(options: { page: number; limit: number }): Promise<OrdersResult> {
  ...
  async getStats(): Promise<OrderStats> {
    ...
    const completedOrders = allOrders.orders.filter(order => order.ord_is_pay === '1');
    const pendingOrders = allOrders.orders.filter(order => order.ord_is_pay === '0');
