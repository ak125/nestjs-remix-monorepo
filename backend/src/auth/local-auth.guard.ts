...
export class LocalAuthGuard extends AuthGuard('local') {
  ...
  handleRequest(err: any, user: any, info: any, context: ExecutionContext) {
    ...
    if (err || !user) {
      ...
      let errorMessage = 'L\'email ou le mot de passe que vous avez saisi est incorrect.';
      ...
      if (err?.message?.includes('Trop de tentatives')) {
        ...
        errorDetails = 'Votre compte est temporairement bloqué. Veuillez réessayer dans quelques minutes.';
      }
      
      // Vérifier si c'est un problème d'utilisateur inactif
      if (err?.message?.includes('inactif') || err?.message?.includes('désactivé')) {
        errorType = 'account_disabled';
        errorMessage = 'Votre compte est désactivé.';
        errorDetails = 'Veuillez contacter l\'administrateur pour réactiver votre compte.';
      }
      
      // Vérifier si c'est un problème d'email non trouvé
      if (err?.message?.includes('email') && err?.message?.includes('invalide')) {
        errorType = 'email_not_found';
        errorMessage = 'Aucun compte associé à cette adresse email.';
        errorDetails = 'Vérifiez l\'orthographe de votre email ou créez un nouveau compte.';
      }
      ...
      return response.redirect(`/login?error=${errorType}&message=${encodeURIComponent(errorMessage)}&email=${encodeURIComponent(email)}`);
    }
