-- ============================================================================
-- MIGRATION: Extract SEO processing from rm_get_page_complete_v2
-- ============================================================================
-- Objectif: Supprimer les 5 appels a process_seo_template() (700-800ms)
-- Solution: Retourner les templates bruts + contexte, traitement par NestJS
--           SeoTemplateService (25-50ms, 0ms si cache Redis hit)
--
-- Date: 2026-04-08
-- Version: v2.2.0
-- Performance target: ~200ms (vs 2133ms avant)
-- ============================================================================
...
BEGIN
    ...
    -- =========================================================================
    -- PARTIE 0: RECUPERER CONTEXTE VEHICULE
    -- =========================================================================
    SELECT
    ...
    -- Recuperer mf_id pour les switches famille
    ...
    -- =========================================================================
    -- NOTE v2.2.0: SEO template processing REMOVED from RPC
    -- Delegated to NestJS SeoTemplateService (25-50ms vs 700-800ms PL/pgSQL)
    -- Raw templates + context returned in seo_raw / seo_context keys
    -- =========================================================================

    -- =========================================================================
    -- PARTIE 2: DONNEES PRINCIPALES (CTEs)
    -- =========================================================================
    WITH
    ...
    -- =========================================================================
    -- PARTIE 3: PIECES VIA FONCTION OPTIMISEE (1.4ms au lieu de 900ms)
    -- =========================================================================
    sorted_pieces AS (
        SELECT
            id,
    ...
    oem_refs_with_position AS (
        SELECT
            ...
            CASE
                ...
                WHEN sp.filtre_side = 'Arriere' THEN 2
    ...
    -- =========================================================================
    -- PARTIE 7: CROSS-SELLING (gammes liees)
    -- =========================================================================
    ...
    SELECT jsonb_build_object(
        ...
        'grouped_pieces', COALESCE((
            SELECT jsonb_agg(
                ...
                ORDER BY
                    ...
                    CASE group_side WHEN 'Avant' THEN 1 WHEN 'Arriere' THEN 2
        ...
        -- RAW SEO TEMPLATES (processing delegated to NestJS SeoTemplateService)
        'seo_raw', (
            SELECT jsonb_build_object(
                'h1', COALESCE(sgc_h1, ''),
        ...
        -- SEO CONTEXT (for NestJS template variable replacement)
        'seo_context', jsonb_build_object(
            'type_id', p_vehicle_id::INTEGER,
            'pg_id', p_gamme_id,
            'mf_id', v_mf_id,
        ...
        -- Placeholder SEO (populated by NestJS before HTTP response)
        ...
        'filters', jsonb_build_object(
            ...
            'sides', COALESCE((
                SELECT jsonb_agg(
                    ...
                    ORDER BY CASE side WHEN 'Avant' THEN 1 WHEN 'Arriere' THEN 2
                        WHEN 'Gauche' THEN 3 WHEN 'Droite' THEN 4 ELSE 5 END
                ) FROM side_counts
            ), '[]'::jsonb),
...
'v2.2.0: Complete RM Page Data RPC - SEO extraction to NestJS

OPTIMISATION v2.2.0:
- SEO template processing extracted to NestJS SeoTemplateService (700ms -> 25ms)
- Returns seo_raw + seo_context for NestJS processing
- Performance: 2133ms -> ~200ms (10x speedup)
...
Response includes:
  ...
  - gamme: Gamme info
  - seo_raw: Raw SEO templates (unprocessed)
  - seo_context: Vehicle/gamme context for template processing
  - seo: Empty placeholder (populated by NestJS SeoTemplateService)
