-- ============================================================================
-- MIGRATION: Fix Position Detection in get_listing_products_extended
-- ============================================================================
-- Problem: Position (Avant/Arrière) is missing from product listings
-- Cause: get_listing_products_extended only has 3 fallbacks, missing criteria tables
-- Solution: Add pieces_criteria and pieces_relation_criteria detection (like V4)
--
-- Date: 2026-01-28
-- Impact: Adds 2 CTEs + 2 JOINs for position detection
-- ============================================================================
...
AS $function$
    WITH
    cdn AS (SELECT 'https://www.automecanik.com/img/v1/object/public' as base),
    ...
    -- =========================================================================
    -- FIX 2026-01-28: Add position detection from criteria tables
    -- =========================================================================

    -- Détection position depuis pieces_criteria (cri_id = 100 = "Emplacement")
    criteria_positions AS (
        SELECT DISTINCT ON (pc_piece_id::INTEGER)
            ...
            CASE
                ...
                WHEN LOWER(pc_cri_value) LIKE '%gauche%' THEN 'Gauche'
                WHEN LOWER(pc_cri_value) LIKE '%droit%' THEN 'Droite'
                ELSE NULL
    ...
    -- Détection position depuis pieces_relation_criteria (priorité véhicule)
    ...
    assembled AS (
        SELECT
            ...
            ap.is_accessory,
            -- FIX: 5 fallbacks pour la position (comme V4)
            COALESCE(
                rcp.detected_position,      -- 1. pieces_relation_criteria (priorité véhicule)
                cp.detected_position,       -- 2. pieces_criteria
                sp.psf_side,               -- 3. pieces_side_filtre
                ap.piece_name_side,        -- 4. pieces.piece_name_side
                CASE                       -- 5. Regex fallback
        ...
        -- FIX: Add joins for criteria-based position detection
...
-- Documentation
-- ============================================================================
COMMENT ON FUNCTION get_listing_products_extended IS
'High-performance product listing with RM scoring (OE/EQUIV/ECO, stock status).
FIX 2026-01-28: Added position detection from pieces_criteria and pieces_relation_criteria tables.
Now has 5 fallbacks for position: relation_criteria > criteria > side_filtre > piece_name_side > regex';
