-- RPC: backfill_seo_keywords_type_ids V2
-- Date: 2026-01-30
-- Description: Backfill batch des type_id avec filtrage par pg_id
-- Usage: SELECT * FROM backfill_seo_keywords_type_ids(100, 7)

CREATE OR REPLACE FUNCTION backfill_seo_keywords_type_ids(
  p_batch_size int DEFAULT 100,
  p_pg_id int DEFAULT NULL  -- NOUVEAU: filtrer par pg_id (NULL = tous)
)
RETURNS TABLE(
  processed int,
  matched int,
...
BEGIN
  ...
  FOR v_keyword IN
    ...
    WHERE type = 'vehicle'
      ...
      -- NOUVEAU: filtrage optionnel par pg_id
      AND (p_pg_id IS NULL OR pg_id = p_pg_id)
    ORDER BY id
    LIMIT p_batch_size
  LOOP
    ...
    v_variant_clean := LOWER(TRIM(COALESCE(v_keyword.variant, '')));
    ...
    WHERE
      ...
      (
        ...
        OR LOWER(COALESCE(mo.modele_alias, '')) = v_model_clean
        OR LOWER(COALESCE(mo.modele_alias, '')) LIKE v_model_clean || '%'
      )
      -- Match variant dans type_name ou type_engine (optionnel si variant vide)
      AND (
        v_variant_clean = ''
        OR LOWER(t.type_name) LIKE '%' || REPLACE(v_variant_clean, ' ', '%') || '%'
    ...
    ORDER BY
      ...
      CASE
        ...
        WHEN LOWER(COALESCE(mo.modele_alias, '')) = v_model_clean THEN 1
        ELSE 2
      END,
      -- Priorite 2: match exact sur variant dans type_name
      CASE
        WHEN v_variant_clean != '' AND LOWER(t.type_name) LIKE '%' || v_variant_clean || '%' THEN 0
        ELSE 1
      END,
...
COMMENT ON FUNCTION backfill_seo_keywords_type_ids(int, int) IS 'Backfill automatique des type_id manquants - V2 avec filtrage par pg_id';
