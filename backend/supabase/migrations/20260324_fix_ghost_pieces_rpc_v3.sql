-- ============================================================================
-- Migration: 20260324_fix_ghost_pieces_rpc_v3.sql
-- Purpose: Eliminate ghost pieces (no name, no filtre_gamme) from RPC v3
-- Impact: Fixes HTTP 500 on pages like /pieces/vanne-egr-1145/audi-22/q7-22071/3-0-tdi-34967.html
-- Root cause: 5427 pieces with piece_display=true but piece_name='' and piece_fil_name=NULL
--             cause grouped_pieces entries with title_h2=null → frontend crash on .toLowerCase()
-- Fix: Filter in sorted_pieces CTE + COALESCE safety net on title_h2 in grouped CTE
-- ============================================================================
...
BEGIN
  ...
  active_pieces AS (
    ...
    WHERE p.piece_display = true
      -- ═══ FIX GHOST PIECES: exclure pièces sans nom ou sans catégorie ═══
      AND p.piece_name IS NOT NULL
      AND TRIM(p.piece_name) != ''
      AND p.piece_fil_name IS NOT NULL
      AND TRIM(p.piece_fil_name) != ''
  ),
  ...
  assembled_pieces AS (
    ...
    LEFT JOIN LATERAL (
            SELECT
                ...
                END as detected_position
            FROM pieces_criteria
            WHERE pc_piece_id::INTEGER = ap.piece_id
              AND pc_cri_id = '100'
  ...
  -- ═══════════════════════════════════════════════════════════════════════════
  -- RÉSULTAT FINAL V3 - AVEC GHOST PIECES FIX
  -- ═══════════════════════════════════════════════════════════════════════════
  SELECT jsonb_build_object(
    'vehicle_info', (
      SELECT jsonb_build_object(
    ...
    'success', true,
    'version', 'RPC_V3_SEO_INTEGRATED_FIXED_V8_GHOST'
  ) INTO v_result;
