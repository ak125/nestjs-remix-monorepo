...
--   2. ENABLE ROW LEVEL SECURITY (or already ON for pieces_ref_ean)
--   3. DROP the two `USING (true)` policies on pieces_ref_ean (advisor flag
--      `rls_policy_always_true`)
--   4. CREATE explicit service_role ALL policy on every table
--
-- Lock impact
-- -----------
...
DROP POLICY IF EXISTS pieces_relation_type_service_role_all ON public.pieces_relation_type;
...
DROP POLICY IF EXISTS pieces_ref_search_service_role_all ON public.pieces_ref_search;
...
DROP POLICY IF EXISTS "Enable insert access for all users" ON public.pieces_ref_ean;
DROP POLICY IF EXISTS "Enable read access for all users"   ON public.pieces_ref_ean;
ALTER TABLE public.pieces_ref_ean ENABLE ROW LEVEL SECURITY;  -- already enabled, idempotent
DROP POLICY IF EXISTS pieces_ref_ean_service_role_all ON public.pieces_ref_ean;
...
COMMENT ON TABLE public.pieces_ref_ean IS
  'Piece EAN/barcode mapping (~3M rows). RLS hardened 2026-04-22 — removed `USING (true)` policies, service_role only.';
...
DROP POLICY IF EXISTS __cnit_raw_service_role_all ON public.__cnit_raw;
...
--   -- expected : exactly 1 service_role policy per table, no `{public}` roles
--
-- =============================================================================
