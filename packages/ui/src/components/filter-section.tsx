...
const FilterSection = React.forwardRef<HTMLDivElement, FilterSectionProps>(
  ({ className, variant, title, icon, badge, headerVariant = 'default', children, ...props }, ref) => {
    return (
      <div
        ref={ref}
        ...
        {...props}
      >
        <div className={cn(filterSectionHeaderVariants({ variant: headerVariant }), 'mb-3')}>
...
interface FilterOptionProps extends React.HTMLAttributes<HTMLLabelElement>,
...
const FilterOption = React.forwardRef<HTMLLabelElement, FilterOptionProps>(
  ({ className, state, isSelected, isDisabled, children, ...props }, ref) => {
    ...
    const computedState = state || (isDisabled ? 'disabled' : isSelected ? 'selected' : 'default');
    
    return (
      <label
        ref={ref}
        className={cn(filterOptionVariants({ state: computedState }), className)}
        {...props}
      >
        {children}
      </label>
...
export { FilterSection, FilterOption, filterSectionVariants, filterOptionVariants };
