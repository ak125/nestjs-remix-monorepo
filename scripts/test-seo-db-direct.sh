...
echo "ðŸ“¦ Test 1: Kit d'embrayage (pg_id=479)"
echo "--------------------------------------"
...
echo "ðŸ”‹ Test 2: Batterie (pg_id=1)"
echo "--------------------------------------"
...
curl -s "$SUPABASE_URL/__seo_gamme_car?select=sgc_h1,sgc_content&limit=5" \
  -H "apikey: $APIKEY" \
  -H "Authorization: Bearer $APIKEY" | jq -r '.[] | select(.sgc_h1 | test("<p")) | "âŒ H1 avec <p>: " + .sgc_h1[:80]' | head -3

echo ""

curl -s "$SUPABASE_URL/__seo_gamme_car?select=sgc_content&limit=5" \
  -H "apikey: $APIKEY" \
  -H "Authorization: Bearer $APIKEY" | jq -r '.[] | select(.sgc_content | test("<p[^>]*>")) | "âš ï¸  Content avec <p>: " + .sgc_content[:80]' | head -3
