#!/bin/bash

# 🔍 Script d'analyse des requêtes lentes
# Usage: ./scripts/analyze-slow-requests.sh

set -e

DOMAIN="${1:-https://automecanik.fr}"

echo "═══════════════════════════════════════════════════════════"
echo "🔍 Analyse des Requêtes Lentes - automecanik.fr"
echo "═══════════════════════════════════════════════════════════"
echo ""

# Routes à tester
routes=(
  "/"
  "/health"
  "/catalog"
  "/api"
  "/products"
  "/search"
)

echo "📊 Test de performance sur toutes les routes"
echo "────────────────────────────────────────────────────────────"
echo ""

# Fichier temporaire pour stocker les résultats
TEMP_FILE="/tmp/route_perf_$$"
> "$TEMP_FILE"

for route in "${routes[@]}"; do
  url="${DOMAIN}${route}"
  echo "🔗 Test: $route"
  
  # Faire 5 requêtes pour avoir une moyenne
  total_time=0
  success_count=0
  
  for i in {1..5}; do
    result=$(curl -s -o /dev/null -w "%{time_total}:%{http_code}" "$url" 2>/dev/null || echo "0:000")
    time=$(echo "$result" | cut -d':' -f1)
    code=$(echo "$result" | cut -d':' -f2)
    
    if [ "$code" = "200" ] || [ "$code" = "304" ]; then
      total_time=$(echo "$total_time + $time" | bc)
      success_count=$((success_count + 1))
    fi
  done
  
  if [ $success_count -gt 0 ]; then
    avg_time=$(echo "scale=3; $total_time / $success_count" | bc)
    echo "   ✅ Moyenne: ${avg_time}s (${success_count}/5 succès)"
    echo "${route}:${avg_time}" >> "$TEMP_FILE"
  else
    echo "   ❌ Route inaccessible ou erreur"
  fi
  echo ""
done

echo "────────────────────────────────────────────────────────────"
echo "🏆 Classement des routes (du plus rapide au plus lent)"
echo "────────────────────────────────────────────────────────────"
echo ""

# Trier par temps
sort -t':' -k2 -n "$TEMP_FILE" | while IFS=: read -r route time; do
  if (( $(echo "$time < 0.1" | bc -l) )); then
    emoji="🚀"
    status="Excellent"
  elif (( $(echo "$time < 0.3" | bc -l) )); then
    emoji="✅"
    status="Très bon"
  elif (( $(echo "$time < 0.5" | bc -l) )); then
    emoji="👍"
    status="Bon"
  elif (( $(echo "$time < 1.0" | bc -l) )); then
    emoji="⚠️ "
    status="Moyen"
  else
    emoji="❌"
    status="Lent"
  fi
  
  printf "%s %-20s %6ss   %s\n" "$emoji" "$route" "$time" "$status"
done

# Identifier les requêtes lentes
echo ""
echo "────────────────────────────────────────────────────────────"
echo "⚠️  Requêtes à optimiser (> 500ms)"
echo "────────────────────────────────────────────────────────────"
echo ""

slow_found=0
while IFS=: read -r route time; do
  if (( $(echo "$time > 0.5" | bc -l) )); then
    printf "⚠️  %s : %ss - À OPTIMISER\n" "$route" "$time"
    slow_found=1
  fi
done < "$TEMP_FILE"

if [ $slow_found -eq 0 ]; then
  echo "✅ Aucune requête lente détectée ! Toutes les routes < 500ms"
fi

# Nettoyage
rm -f "$TEMP_FILE"

echo ""
echo "═══════════════════════════════════════════════════════════"
echo "✅ Analyse terminée"
echo "═══════════════════════════════════════════════════════════"
