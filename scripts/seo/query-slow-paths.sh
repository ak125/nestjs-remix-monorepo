...
# Paramètres
THRESHOLD=${1:-800}   # Seuil en ms (défaut: 800ms)
LIMIT=${2:-50}        # Nombre de résultats (défaut: 50)
DAY=${3:-$(date +%Y-%m-%d)}  # Jour (défaut: aujourd'hui)

echo -e "${RED}�� Chemins lents (> ${THRESHOLD}ms) - ${DAY}${NC}\n"
...
("Latence médiane (p50): \(.hits | map(.latency_ms) | sort | .[((length * 0.50) | floor)])ms"),
...
"",
"🔥 TOP 10 ROUTES LES PLUS LENTES (par route pattern):",
"",
([.hits | group_by(.route) | .[] | {
...
SEUILS RECOMMANDÉS:
  - 300ms  : Chemins "moyens" à surveiller
  - 500ms  : Chemins "lents" à optimiser
  - 800ms  : Chemins "très lents" (défaut)
  - 1000ms : Chemins "critiques" (1s+)
  - 2000ms : Chemins "catastrophiques" (2s+)

ACTIONS POSSIBLES:
  1. Identifier routes récurrentes (pattern /pieces/:brand/:gamme)
  2. Vérifier si DB queries lentes sur certaines marques
  3. Analyser si latence liée à pays (GeoIP overhead?)
  4. Comparer GET vs POST (API endpoints vs pages)
  5. Corréler avec statut 5xx (erreurs ralentissent?)
═══════════════════════════════════════════════════════════════
HELP
