...
def generate_queries(slug: str, data: dict) -> str:
    ...
    # 1. Transactionnelles (R1_ROUTER — entrée gamme/compatibilité, transition vers R2)
    lines.append(f"\n{'─' * 50}")
    lines.append("  1. TRANSACTIONNELLES (R1_ROUTER)")
    lines.append(f"{'─' * 50}")
    ...
    # 2. Informationnelles (R3_CONSEILS — how-to / pédagogie prudente)
    lines.append(f"\n{'─' * 50}")
    lines.append("  2. INFORMATIONNELLES (R3_CONSEILS)")
    lines.append(f"{'─' * 50}")
    ...
    # 3. Guide-achat (R6_GUIDE_ACHAT — comparatif / critères avant achat, cible S3)
    lines.append(f"\n{'─' * 50}")
    lines.append("  3. GUIDE-ACHAT (R6_GUIDE_ACHAT — cible S3)")
    lines.append(f"{'─' * 50}")
