...
echo -e "${BLUE}════════════════════════════════════════${NC}"
echo -e "${BLUE}    ÉTAT DES SERVICES - DEV${NC}"
echo -e "${BLUE}════════════════════════════════════════${NC}"
echo ""

# Vérifier Backend/Frontend (port 3000)
echo -e "${YELLOW}🔍 Backend + Frontend (port 3000)${NC}"
...
if docker ps --filter "name=redis" --format "{{.Names}}" | grep -q redis; then
    ...
    echo -e "${RED}❌ NON ACTIF${NC}"
    echo "   Démarrer avec: docker run -d --name redis-dev --rm -p 6379:6379 redis:7-alpine"
fi
echo ""

# Processus Node.js actifs
echo -e "${YELLOW}🔍 Processus Node.js${NC}"
NODE_PROCESSES=$(ps aux | grep node | grep -v grep | grep -v "vscode" | grep -v "copilot" | wc -l)
if [ $NODE_PROCESSES -gt 0 ]; then
    echo -e "${GREEN}✅ $NODE_PROCESSES processus actifs${NC}"
    ps aux | grep node | grep -v grep | grep -v "vscode" | grep -v "copilot" | head -5 | awk '{print "   PID "$2" - "$11" "$12" "$13}'
else
    echo -e "${YELLOW}⚠️  Aucun processus Node.js détecté${NC}"
fi
echo ""
...
if [ $DOCKER_CONTAINERS -gt 0 ]; then
    echo -e "${GREEN}✅ $DOCKER_CONTAINERS conteneur(s) actif(s)${NC}"
...
if [ $SCORE -eq 2 ]; then
    ...
    echo -e "${YELLOW}⚠️  SERVICES PARTIELLEMENT ACTIFS ($SCORE/2)${NC}"
    echo ""
    echo "💡 Commandes utiles :"
    if ! lsof -i :3000 > /dev/null 2>&1; then
        echo "   - Démarrer le backend : npm run dev"
    fi
    if ! docker ps --filter "name=redis" --format "{{.Names}}" | grep -q redis; then
        echo "   - Démarrer Redis     : docker run -d --name redis-dev --rm -p 6379:6379 redis:7-alpine"
    fi
    echo ""
else
