...
echo -e "${BLUE}ðŸ” SEO WEEKLY AUDIT - $(date '+%Y-%m-%d %H:%M:%S')${NC}"
echo -e "${BLUE}â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”${NC}"
echo ""
...
echo -e "${BLUE}ðŸ“‹ [1/5] Validation XSD du sitemap...${NC}"
...
if [ ! -s "$SITEMAP_FILE" ]; then
  ...
  if grep -q "<sitemapindex" "$SITEMAP_FILE"; then
    echo -e "${BLUE}ðŸ“‘ Sitemap Index dÃ©tectÃ©${NC}"
    ...
    echo -e "${BLUE}ðŸ“„ Sitemap standard dÃ©tectÃ©${NC}"
    ...
    TOTAL_URLS=$(wc -l < "$OUTPUT_DIR/sitemap-urls.txt")
    echo -e "${BLUE}ðŸ“Š Total URLs dans sitemap: $TOTAL_URLS${NC}"
  fi
fi
...
echo -e "${BLUE}ðŸš« [2/5] DÃ©tection URLs noindex prÃ©sentes dans sitemap...${NC}"
...
if [ "$NOINDEX_COUNT" -eq 0 ]; then
  ...
  echo -e "${RED}âŒ $NOINDEX_COUNT URLs avec noindex trouvÃ©es (voir $NOINDEX_FILE)${NC}"
  echo -e "${YELLOW}ðŸ’¡ Action: Retirer ces URLs du sitemap ou enlever noindex${NC}"
fi
...
echo -e "${BLUE}ðŸ”´ [3/5] DÃ©tection URLs retournant 4xx/5xx...${NC}"
...
echo -e "${BLUE}ðŸŒ [4/5] Validation hreflang rÃ©ciproques...${NC}"
...
echo -e "${BLUE}ðŸ”— [5/5] Validation canoniques divergents...${NC}"
...
if [ "$CANONICAL_ERROR_COUNT" -eq 0 ]; then
  ...
  echo -e "${BLUE}ðŸ’¡ Note: Peut Ãªtre intentionnel (variantes produits, pagination)${NC}"
fi
...
echo -e "${BLUE}â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”${NC}"
echo -e "${BLUE}ðŸ“Š RAPPORT FINAL${NC}"
echo -e "${BLUE}â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”${NC}"
...
if [ "$TOTAL_ERRORS" -eq 0 ]; then
  ...
  echo -e "${RED}âŒ AUDIT FAILED - $TOTAL_ERRORS erreur(s) critique(s)${NC}"
  EXIT_CODE=1
fi
...
if [ -n "$WEBHOOK_URL" ]; then
  echo -e "${BLUE}ðŸ“¨ Envoi notification webhook...${NC}"
  
  STATUS_EMOJI=$([ $TOTAL_ERRORS -eq 0 ] && echo "âœ…" || echo "âŒ")
  ...
  "blocks": [
    {
      ...
      "text": {
        "type": "mrkdwn",
        "text": "*SEO Weekly Audit Report*\n\n*Status:* $([ $TOTAL_ERRORS -eq 0 ] && echo "PASS âœ…" || echo "FAIL âŒ")\n*Total URLs:* $TOTAL_URLS\n*Errors:* $TOTAL_ERRORS\n*Warnings:* $TOTAL_WARNINGS"
      }
    },
    {
      "type": "section",
