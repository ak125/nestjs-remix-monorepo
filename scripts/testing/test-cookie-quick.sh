...
# 🧪 Test Rapide - Cookie Véhicule

echo "🧪 Test 1: Requête SANS cookie"
echo "─────────────────────────────────"
curl -s "http://localhost:3000/pieces/filtre-a-huile-12.html" | \
  grep -oP '(?<=🍞 Breadcrumb: ).*' || echo "Logs non détectés (serveur peut-être pas en mode dev)"

echo ""
echo "🧪 Test 2: Requête AVEC cookie Renault Avantime"
echo "────────────────────────────────────────────────"

# Cookie véhicule encodé
...
curl -s -H "Cookie: $COOKIE" "http://localhost:3000/pieces/filtre-a-huile-12.html" | \
  grep -oP '(?<=🍞 Breadcrumb: ).*' || echo "Logs non détectés"
