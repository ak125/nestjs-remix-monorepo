...
# 🔍 Script de validation de cohérence URL ↔ Breadcrumb
...
echo -e "${BLUE}${BOLD}🔍 Validation Cohérence URL ↔ Breadcrumb${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
# Télécharger la page
echo -e "${BLUE}⬇️  Téléchargement de la page...${NC}"
if ! curl -s -L "$URL" -o "$TEMP_FILE"; then
  echo -e "${RED}❌ Impossible de télécharger l'URL${NC}"
  rm -f "$TEMP_FILE"
  exit 1
fi

# Extraire le JSON-LD du breadcrumb
echo -e "${BLUE}📊 Extraction du Schema.org BreadcrumbList...${NC}"
...
if [[ -z "$BREADCRUMB_JSON" ]]; then
  echo -e "${RED}❌ Aucun BreadcrumbList trouvé dans la page${NC}"
  rm -f "$TEMP_FILE"
  exit 1
fi

# Parser les éléments du breadcrumb
BREADCRUMB_ITEMS=$(echo "$BREADCRUMB_JSON" | jq -r '.itemListElement[] | "\(.position)|\(.name)|\(.item)"' 2>/dev/null || echo "")

if [[ -z "$BREADCRUMB_ITEMS" ]]; then
  echo -e "${RED}❌ Impossible de parser le BreadcrumbList${NC}"
  rm -f "$TEMP_FILE"
  exit 1
fi

echo -e "${GREEN}✅ BreadcrumbList trouvé${NC}"
echo ""

# Afficher le breadcrumb
echo -e "${BOLD}📍 Fil d'Ariane (Schema.org):${NC}"
echo "$BREADCRUMB_ITEMS" | while IFS='|' read -r pos name url; do
  echo -e "  ${YELLOW}[$pos]${NC} $name ${BLUE}→${NC} $url"
done
echo ""

# Extraire les segments de l'URL
echo -e "${BOLD}🔗 Segments URL:${NC}"
...
echo -e "${BOLD}🎯 Validation de Cohérence:${NC}"
echo ""
...
# Vérifier si segment 2 URL apparaît avant segment 3 dans le breadcrumb
BREADCRUMB_ORDER=$(echo "$BREADCRUMB_LABELS" | grep -n ".*" | tr '\n' '|')

if [[ "$URL_PATH" =~ ^/pieces/.+/.+/.+/.+ ]]; then
  # URL de type /pieces/{gamme}/{marque}/{modele}/{type}
  echo -e "  ${BLUE}Type détecté:${NC} Pièces par véhicule"
  
  # Breadcrumb attendu: Gamme → Véhicule → Résultat
  ...
  echo -e "  ${YELLOW}URL Segment[2]:${NC} $URL_SEGMENT_3 (véhicule)"
  
  # Vérifier que gamme apparaît avant véhicule
  if echo "$FIRST_BREADCRUMB" | grep -iq "$URL_SEGMENT_2"; then
    if echo "$SECOND_BREADCRUMB" | grep -iq "$URL_SEGMENT_3"; then
      echo -e "  ${GREEN}✅ Ordre correct : Gamme → Véhicule${NC}"
    else
      echo -e "  ${RED}❌ Véhicule non trouvé en 2e position${NC}"
      ((ISSUES++))
    fi
  else
    echo -e "  ${YELLOW}⚠️  Gamme non trouvée en 1ère position (vérification manuelle requise)${NC}"
    ((WARNINGS++))
  fi
  
elif [[ "$URL_PATH" =~ ^/constructeurs/.+/.+/.+ ]]; then
  # URL de type /constructeurs/{marque}/{modele}/{type}
  echo -e "  ${BLUE}Type détecté:${NC} Véhicule"
  echo -e "  ${GREEN}✅ Structure attendue : Constructeurs → Marque → Modèle${NC}"
  
elif [[ "$URL_PATH" =~ ^/pieces/.+ ]]; then
  # URL de type /pieces/{gamme}
  echo -e "  ${BLUE}Type détecté:${NC} Pièces par gamme"
  echo -e "  ${GREEN}✅ Structure attendue : Pièces → Gamme${NC}"
fi
...
# Vérifier que chaque segment URL a un label similaire dans le breadcrumb
for i in "${!SEGMENTS[@]}"; do
  ...
  if echo "$BREADCRUMB_LABELS" | grep -iq "$normalized"; then
    echo -e "  ${GREEN}✅${NC} /$segment → $normalized (trouvé)"
  else
    echo -e "  ${YELLOW}⚠️${NC}  /$segment → $normalized (non trouvé, vérification manuelle)"
    ((WARNINGS++))
  fi
done
...
# Test 3: Premier élément = "Accueil"
echo -e "${BLUE}Test 3:${NC} Premier élément = 'Accueil'"
FIRST_ITEM=$(echo "$BREADCRUMB_ITEMS" | head -n1 | awk -F'|' '{print $2}')
if [[ "$FIRST_ITEM" == "Accueil" ]]; then
  echo -e "  ${GREEN}✅ Premier élément = 'Accueil'${NC}"
else
  echo -e "  ${RED}❌ Premier élément = '$FIRST_ITEM' (devrait être 'Accueil')${NC}"
  ((ISSUES++))
fi

echo ""

# Test 4: Positions séquentielles
echo -e "${BLUE}Test 4:${NC} Positions séquentielles"
...
while IFS= read -r pos; do
  if [[ "$pos" -ne "$EXPECTED_POS" ]]; then
    echo -e "  ${RED}❌ Position $pos trouvée, $EXPECTED_POS attendue${NC}"
    IS_SEQUENTIAL=false
    ((ISSUES++))
  fi
  ((EXPECTED_POS++))
...
if $IS_SEQUENTIAL; then
  echo -e "  ${GREEN}✅ Positions séquentielles (1, 2, 3, ...)${NC}"
fi
...
if [[ "$RELATIVE_URLS" -gt 0 ]]; then
  echo -e "  ${RED}❌ $RELATIVE_URLS URL(s) relative(s) trouvée(s) (doivent être absolues)${NC}"
  ((ISSUES++))
else
  echo -e "  ${GREEN}✅ Toutes les URLs sont absolues${NC}"
fi

echo ""

# Test 6: Dernier élément sans lien
echo -e "${BLUE}Test 6:${NC} Dernier élément sans lien ou lien vers page actuelle"
LAST_URL=$(echo "$BREADCRUMB_ITEMS" | tail -n1 | awk -F'|' '{print $3}')

if [[ -z "$LAST_URL" ]] || [[ "$LAST_URL" == "$URL" ]] || [[ "$LAST_URL" == "null" ]]; then
  echo -e "  ${GREEN}✅ Dernier élément sans lien${NC}"
else
  echo -e "  ${YELLOW}⚠️  Dernier élément a un lien : $LAST_URL${NC}"
...
if [[ "$ISSUES" -eq 0 ]] && [[ "$WARNINGS" -eq 0 ]]; then
  ...
  echo -e "${GREEN}Cohérence URL ↔ Breadcrumb : 100%${NC}"
  SCORE=100
elif [[ "$ISSUES" -eq 0 ]]; then
  echo -e "${YELLOW}${BOLD}⚠️  VALIDATION AVEC AVERTISSEMENTS${NC}"
  echo -e "${YELLOW}$WARNINGS avertissement(s) - Vérification manuelle recommandée${NC}"
  SCORE=75
else
...
echo ""
echo -e "Score de cohérence : ${BOLD}$SCORE%${NC}"
