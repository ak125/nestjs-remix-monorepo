#!/bin/bash

################################################################################
# 🔍 Script de Vérification URLs - Version Simplifiée
# 
# Utilise directement l'API de compatibilité
#
# Usage :
#   bash scripts/verify-url-compatibility-simple.sh
#   bash scripts/verify-url-compatibility-simple.sh 100
################################################################################

set -e

# Couleurs
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

# Configuration
API_BASE_URL=${API_BASE_URL:-"http://localhost:3000"}
SAMPLE_SIZE=${1:-50}
OUTPUT_DIR="/tmp"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)

echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}🔍 VÉRIFICATION COMPATIBILITÉ URLs${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo -e "${YELLOW}Configuration :${NC}"
echo "  API Base URL : $API_BASE_URL"
echo "  Sample size  : $SAMPLE_SIZE gammes"
echo ""

# Test connexion API
echo -e "${YELLOW}📊 Test connexion API...${NC}"
if ! curl -s -f "$API_BASE_URL/api/seo/url-compatibility/report" > /dev/null; then
  echo -e "${RED}❌ Erreur : API non accessible${NC}"
  echo ""
  echo "Vérifiez que :"
  echo "  - Le backend est démarré sur $API_BASE_URL"
  echo "  - Le service UrlCompatibilityService est chargé"
  echo ""
  exit 1
fi
echo -e "${GREEN}✅ API accessible${NC}"
echo ""

# Récupérer rapport complet
echo -e "${YELLOW}📊 Étape 1/3 : Récupération du rapport complet${NC}"
REPORT=$(curl -s "$API_BASE_URL/api/seo/url-compatibility/report")

if [ -z "$REPORT" ]; then
  echo -e "${RED}❌ Erreur : Rapport vide${NC}"
  exit 1
fi

echo "$REPORT" > "$OUTPUT_DIR/url-report-$TIMESTAMP.json"
echo -e "${GREEN}✅ Rapport récupéré${NC}"
echo ""

# Vérification détaillée
echo -e "${YELLOW}📊 Étape 2/3 : Vérification détaillée ($SAMPLE_SIZE gammes)${NC}"
VERIFICATION=$(curl -s "$API_BASE_URL/api/seo/url-compatibility/verify?type=gammes&sampleSize=$SAMPLE_SIZE")

if [ -z "$VERIFICATION" ]; then
  echo -e "${RED}❌ Erreur : Vérification vide${NC}"
  exit 1
fi

echo "$VERIFICATION" > "$OUTPUT_DIR/url-verification-$TIMESTAMP.json"
echo -e "${GREEN}✅ Vérification terminée${NC}"
echo ""

# Extraire statistiques
echo -e "${YELLOW}📊 Étape 3/3 : Analyse des résultats${NC}"
echo ""

TOTAL=$(echo "$VERIFICATION" | jq -r '.data.summary.total')
EXACT_MATCH=$(echo "$VERIFICATION" | jq -r '.data.summary.exact_match')
ALIAS_MISSING=$(echo "$VERIFICATION" | jq -r '.data.summary.alias_missing')
MATCH_RATE=$(echo "$VERIFICATION" | jq -r '.data.summary.match_rate')

TOTAL_GAMMES=$(echo "$REPORT" | jq -r '.data.gammes.total')
WITH_ALIAS=$(echo "$REPORT" | jq -r '.data.gammes.with_alias')
WITHOUT_ALIAS=$(echo "$REPORT" | jq -r '.data.gammes.without_alias')

echo "📊 STATISTIQUES GLOBALES :"
echo ""
echo "  Total gammes            : $TOTAL_GAMMES"
echo "  Avec alias              : $WITH_ALIAS"
echo "  Sans alias              : $WITHOUT_ALIAS"
echo ""

echo "📊 RÉSULTATS VÉRIFICATION :"
echo ""
echo "  URLs testées            : $TOTAL"
echo -e "  ${GREEN}✅ Correspondance exacte : $EXACT_MATCH${NC}"
if [ "$ALIAS_MISSING" -gt 0 ]; then
  echo -e "  ${YELLOW}⚠️  Alias manquants       : $ALIAS_MISSING${NC}"
fi
echo -e "  ${BLUE}Taux matching           : $MATCH_RATE%${NC}"
echo ""

# Afficher exemples
echo -e "${YELLOW}🔍 EXEMPLES D'URLs GÉNÉRÉES :${NC}"
echo ""
echo "$REPORT" | jq -r '.data.gammes.sample_urls[]' | head -5
echo ""

# Interpréter résultats
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}📄 FICHIERS GÉNÉRÉS${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo "  Rapport complet   : $OUTPUT_DIR/url-report-$TIMESTAMP.json"
echo "  Vérification      : $OUTPUT_DIR/url-verification-$TIMESTAMP.json"
echo ""

echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}💡 INTERPRÉTATION${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""

if [ "$MATCH_RATE" == "100" ]; then
  echo -e "${GREEN}✅ PARFAIT !${NC}"
  echo ""
  echo "Toutes les URLs générées sont IDENTIQUES au format ancien sitemap."
  echo ""
  echo "Prochaines étapes :"
  echo "  1. Générer le sitemap : curl http://localhost:3000/api/sitemap/products.xml"
  echo "  2. Lancer audit crawl budget"
  echo "  3. Créer expérience A/B"
  echo ""
elif (( $(echo "$MATCH_RATE >= 95" | bc -l) )); then
  echo -e "${GREEN}✅ EXCELLENT${NC}"
  echo ""
  echo "Taux de matching : $MATCH_RATE%"
  echo ""
  if [ "$WITHOUT_ALIAS" -gt 0 ]; then
    echo "Actions recommandées :"
    echo "  - $WITHOUT_ALIAS gammes sans alias détectées"
    echo "  - Générer alias manquants (voir guide)"
  fi
  echo ""
else
  echo -e "${YELLOW}⚠️  BON MAIS À AMÉLIORER${NC}"
  echo ""
  echo "Taux de matching : $MATCH_RATE%"
  echo ""
  echo "Actions recommandées :"
  echo "  - Analyser les différences dans les fichiers JSON"
  echo "  - Corriger les alias manquants"
  echo "  - Re-tester après corrections"
  echo ""
fi

echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}🔧 COMMANDES UTILES${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo "# Voir rapport complet"
echo "cat $OUTPUT_DIR/url-report-$TIMESTAMP.json | jq"
echo ""
echo "# Voir détails vérification"
echo "cat $OUTPUT_DIR/url-verification-$TIMESTAMP.json | jq '.data.details[0:10]'"
echo ""
echo "# Tester avec plus de gammes"
echo "bash scripts/verify-url-compatibility-simple.sh 500"
echo ""

exit 0
