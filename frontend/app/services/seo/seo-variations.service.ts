...
export async function getPrixPasCherVariation(
  ...
  try {
    ...
    console.error('❌ Erreur récupération variation prix:', error);
    return "au meilleur prix";
  }
}
...
export async function getH1Variation(
  ...
  try {
    ...
    console.error('❌ Erreur récupération variation H1:', error);
    return "";
  }
}
...
export async function getSeoVariation(
  ...
  try {
    ...
    const index = calculateVariationIndex(typeId, pgId, variations.length, offset);
    
    return variations[index]?.sis_content || "";
  } catch (error) {
    console.error(`❌ Erreur récupération variation alias ${alias}:`, error);
    return "";
  }
}
