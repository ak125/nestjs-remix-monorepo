...
import { type Order, type OrdersStats, type OrderFilters } from '../../types/orders.types';
...
export async function markOrderPaid(
  ...
  if (!response.ok) {
    throw new Error(`Erreur lors du marquage comme payée: ${response.statusText}`);
  }
...
export async function cancelOrder(
  ...
  if (!response.ok) {
    throw new Error(`Erreur lors de l'annulation de la commande: ${response.statusText}`);
  }
...
export async function generateInvoicePDF(orderId: string): Promise<Blob> {
  ...
  if (!response.ok) {
    throw new Error(`Erreur lors de la génération de la facture: ${response.statusText}`);
  }
...
export async function sendOrderConfirmationEmail(
  ...
  const response = await fetch(`${API_BASE_URL}/orders/${orderId}/send-confirmation`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  });
