...
export const cartApi = {
   ...
   */
  async addItem(productId: number, quantity: number = 1): Promise<AddItemResponse> {
    try {
      ...
      if (!response.ok) {
        console.error('❌ [cartApi.addItem] Erreur HTTP:', response.status, data);
        return {
          success: false,
   ...
   */
  async updateQuantity(productId: number, quantity: number): Promise<UpdateQuantityResponse> {
    try {
      ...
      if (!response.ok) {
        console.error('❌ [cartApi.updateQuantity] Erreur HTTP:', response.status, data);
        return {
          success: false,
          error: data.message || `Erreur HTTP ${response.status}`,
        };
  ...
  async removeItem(productId: number): Promise<RemoveItemResponse> {
    try {
      ...
      if (!response.ok) {
        console.error('❌ [cartApi.removeItem] Erreur HTTP:', response.status, data);
        return {
          success: false,
          error: data.message || `Erreur HTTP ${response.status}`,
        };
  ...
  async clearCart(): Promise<ClearCartResponse> {
    try {
      ...
      if (!response.ok) {
        console.error('❌ [cartApi.clearCart] Erreur HTTP:', response.status, data);
        return {
          success: false,
          error: data.message || `Erreur HTTP ${response.status}`,
        };
