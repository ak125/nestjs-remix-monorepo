  ...
  if (modelId || typeId || year) {
    ...
    const productVariations = [
      ...
      },
      {
        ...
        category: "Moteur",
        description:
          "Kit complet courroie de distribution avec tendeur et galets",
        availability: "En stock",
        rating: 4.9,
      },
      {
...
export default function AdvancedVehicleSearch() {
  const { compatibleProducts, stats, searchParams } =
  ...
  const categories = useMemo(() => {
    const uniqueCategories = [
      ...new Set(compatibleProducts.map((p) => p.category).filter(Boolean)),
    ] as string[];
  ...
  const handleSaveSearch = useCallback(async () => {
    ...
    try {
      ...
      const savedSearches = JSON.parse(
        localStorage.getItem("vehicleSearches") || "[]",
      );
      savedSearches.unshift(searchCriteria);
      localStorage.setItem(
        "vehicleSearches",
        JSON.stringify(savedSearches.slice(0, 10)),
  ...
  const handleSearch = (modelId?: string, typeId?: string, year?: string) => {
    ...
    const finalModelId =
      modelId || (selectedModel ? selectedModel.modele_id.toString() : "");
    const finalTypeId =
      typeId || (selectedType ? selectedType.type_id.toString() : "");
  ...
  const completedSteps = [selectedModel, selectedType, selectedYear].filter(
    Boolean,
  ).length;
  const totalSteps = 3;
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1 flex items-center gap-2">
              ...
              {stats?.totalProducts?.toLocaleString() || 0} pièces disponibles
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            {favorites.length > 0 && (
              ...
              >
                <Heart
                  className={`h-4 w-4 ${favorites.length > 0 ? "fill-current" : ""}`}
                />
            ...
            <button
              ...
              disabled={
                isSaving || !(selectedModel || selectedType || selectedYear)
              }
          ...
          <div className="space-y-2">
            ...
            {selectedModel && (
              <div className="flex items-center text-sm text-green-600">
                ...
                Modèle sélectionné: {selectedModel.auto_marque?.marque_name}{" "}
                {selectedModel.modele_name}
              </div>
            )}
          </div>
            ...
            <div className="space-y-2 animate-fadeIn">
              ...
              {selectedType && (
                <div className="flex items-center text-sm text-green-600">
                  ...
                  Motorisation sélectionnée: {selectedType.type_name} (
                  {selectedType.type_engine_code || "N/A"})
                </div>
              )}
            </div>
          )}
              ...
              {selectedModel && (
                <div className="bg-white p-3 rounded border">
                  ...
                  <div className="font-medium text-gray-900">
                    {selectedModel.auto_marque?.marque_name}{" "}
                    {selectedModel.modele_name}
                  </div>
                </div>
              )}
              {selectedType && (
                <div className="bg-white p-3 rounded border">
                  ...
                  <div className="text-sm text-gray-600">
                    Code: {selectedType.type_engine_code || "N/A"}
                  </div>
                </div>
              )}
              {selectedYear && (
          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredAndSortedProducts.map((product) => (
                <div
                  key={product.id}
                  className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-lg hover:border-blue-300 transition-all duration-200 group"
                >
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="font-medium text-gray-900 line-clamp-2 group-hover:text-blue-700 transition-colors">
                      {product.name}
                    </h3>
        ...
        {searchParams.modelId && compatibleProducts.length === 0 && (
          <div className="mt-8 text-center py-12 bg-gray-50 rounded-lg">
            ...
            <p className="text-gray-600 mb-6">
              Aucune pièce compatible n'a été trouvée pour les critères
              sélectionnés.
            </p>
            <button
