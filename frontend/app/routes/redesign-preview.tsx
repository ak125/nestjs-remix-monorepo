...
export const meta: MetaFunction = () => [
  { title: "AutoMecanik — Preview redesign navy/orange" },
...
export default function RedesignPreview() {
  ...
  const [selectedVehicle, _setSelectedVehicle] = useState<string | null>(null);
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Navbar et Footer viennent du layout root.tsx */}
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[9999] flex items-center gap-2 px-5 py-2.5 bg-green-700 text-white rounded-xl text-sm font-semibold shadow-2xl animate-in slide-in-from-top-2">
      ...
      <Reveal>
        <section className="relative overflow-hidden py-6 sm:py-8 md:py-10 bg-gradient-to-br from-[#0d1b3e] via-[#0f2347] to-[#162d5a]">
          ...
          <div className="relative container mx-auto px-4 max-w-[780px] text-center">
              ...
              <Tabs defaultValue="vehicule">
                ...
                <TabsContent value="mine" className="mt-0 bg-white p-4 sm:p-5">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2.5">
                    ...
                    <Button className="min-h-[46px] rounded-xl px-6 font-bold text-sm uppercase tracking-wide bg-[#e8590c] hover:bg-[#d9480f] text-white shadow-[0_4px_14px_rgba(232,89,12,0.3)] whitespace-nowrap">
            ...
            {/* Vehicle tag — visible apres selection */}
            {selectedVehicle && (
              <Badge
                variant="outline"
                className="mt-3.5 px-3.5 py-1.5 bg-white/10 border-white/[0.12] rounded-full text-xs font-semibold text-white/80 hover:bg-white/15 max-w-full truncate"
              >
                <Car className="w-3.5 h-3.5 text-green-400 mr-1.5" />{" "}
                {selectedVehicle}
              </Badge>
            )}
          </div>
              ...
              <Link to={item.link}>
                <Card className="group relative rounded-2xl overflow-hidden border-[1.5px] hover:border-[#e8590c]/20 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
                  ...
                  <CardContent className="p-3 sm:p-3.5 pt-0 sm:pt-0">
                    ...
                    <Button
                      size="sm"
                      className="w-full mt-2 h-9 text-xs font-semibold bg-[#0d1b3e] hover:bg-[#e8590c] text-white rounded-lg transition-colors"
                      onClick={(e) => {
                        e.preventDefault();
                        addToCart(item.name);
                      }}
