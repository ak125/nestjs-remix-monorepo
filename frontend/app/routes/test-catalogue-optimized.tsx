...
export async function loader({ params, request }: LoaderFunctionArgs) {
  ...
  try {
    // 🚀 Pas de timeout - Redis cache les réponses lentes de Supabase
    // La première requête peut prendre ~20s, les suivantes <10ms grâce au cache
      ...
      fetch(
        ...
        { 
          headers: {
            ...
            'Cache-Control': 'no-cache' // Forcer vérification cache côté serveur
          }
        }
      ),
      // 🏠 Récupérer aussi la hiérarchie pour avoir l'ordre correct des gammes
      fetch(
...
export default function PiecesDetailPage() {
  ...
  } as any) : 'from-blue-950 via-indigo-900 to-purple-900'; // Fallback
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 to-neutral-100">
                  ...
                  <div className="relative group">
                    ...
                    <div className="relative bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-lg group-hover:border-white/40 transition-all duration-500">
                      <div className="w-full aspect-square flex items-center justify-center">
                        <img
                          src="https://cxpojprgwgubzjyqzmoq.supabase.co/storage/v1/object/public/uploads/articles/gammes-produits/catalogue/plaquette-de-frein.webp"
                          alt={data.content?.pg_name || "Plaquette de frein"}
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {/* 🖼️ Galerie d'images produits catalogue - Déplacé après le Guide Expert */}
        {data.catalogueMameFamille && data.catalogueMameFamille.items.length > 0 && (
          <div className="container mx-auto px-4 py-3 md:py-4">
            {/* Cadre avec fond gradient comme la hero section */}
            <div className={`relative rounded-lg overflow-hidden shadow-lg bg-gradient-to-br ${familleColor}`}>
              ...
              <div className="relative p-3 md:p-4">
                <h2 className="text-base md:text-lg font-bold text-white mb-3 text-center drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]">
                  Catalogue {data.famille?.mf_name || "Système de freinage"}
                </h2>
                <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-10 lg:grid-cols-14 xl:grid-cols-16 gap-1.5 auto-rows-max">
                {data.catalogueMameFamille.items.slice(0, 32).map((item, index) => (
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/70 to-transparent text-white text-[6px] sm:text-[7px] p-1 group-hover:from-black/95 group-hover:via-black/85 transition-all duration-300">
                ...
                {data.catalogueMameFamille.items.length > 32 && (
                  <div className="flex items-center justify-center aspect-square rounded-md bg-white/20 backdrop-blur-sm border border-white/30 text-white font-bold text-[9px] md:text-[10px] shadow-sm">
                    +{data.catalogueMameFamille.items.length - 32}
                  </div>
                )}
              </div>
