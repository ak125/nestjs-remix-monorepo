  ...
  try {
    ...
    try {
      const [monitoringRes, healthRes] = await Promise.all([
        fetch("http://127.0.0.1:3000/api/admin/paybox-monitoring").catch(
          () => null,
        ),
        fetch("http://127.0.0.1:3000/api/admin/paybox-health").catch(
          () => null,
        ),
      ]);
...
export default function AdminPaymentsDashboard() {
  const { payments, stats, pagination, payboxMonitoring, payboxHealth } =
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                      <div>
                        ...
                        <span className="ml-2 font-mono font-semibold text-gray-900">
                          {payboxHealth.config.site}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-500">Rang:</span>
                        <span className="ml-2 font-mono font-semibold text-gray-900">
                          {payboxHealth.config.rang}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-500">Identifiant:</span>
                        <span className="ml-2 font-mono font-semibold text-gray-900">
                          {payboxHealth.config.identifiant}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-500">HMAC:</span>
                        <span
                          className={`ml-2 font-semibold ${payboxHealth.config.hmacKey === "CONFIGURED" ? "text-green-600" : "text-red-600"}`}
                        >
                          {payboxHealth.config.hmacKey === "CONFIGURED"
                            ? "✓ Configuré"
                            : "✗ Manquant"}
                        </span>
                      </div>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-blue-700">
                <strong>ℹ️ Vue basée sur les commandes payées</strong> - Affiche
                uniquement les commandes confirmées et payées (hors statut "En
                attente"). Les données proviennent de la table des commandes.
              </p>
            </div>
          </div>
        </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {payments.map((payment) => (
                  <tr key={payment.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusBadge(payment.status)}`}
                      >
                        {payment.status}
                      </span>
                    </td>
