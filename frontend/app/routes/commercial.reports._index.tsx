...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  try {
    ...
    const [dashboardResponse, ordersResponse] = await Promise.all([
      ...
      fetch('http://localhost:3000/api/dashboard/orders/recent', {
        headers: { 'Cookie': request.headers.get('Cookie') || '' },
      }),
    ]);
    ...
    const conversionRate = dashboardData.totalOrders > 0 
      ? ((dashboardData.completedOrders / dashboardData.totalOrders) * 100).toFixed(1)
      : '0.0';

    const averageOrderValue = dashboardData.completedOrders > 0
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                {data.statistics.completedOrders}/{data.statistics.totalOrders} commandes
              </p>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center">
                ...
                <span className="text-sm font-semibold">{data.statistics.completedOrders}</span>
              </div>
            </div>
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center">
                <div className="w-32 bg-gray-200 rounded-full h-2 mr-3">
                  <div 
                    ...
                    style={{width: `${100 - parseFloat(data.statistics.conversionRate)}%`}}
                  ></div>
                </div>
                <span className="text-sm font-semibold">{data.statistics.pendingOrders}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-semibold">{data.statistics.totalSuppliers}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-600">Clients</span>
              <span className="font-semibold">{data.statistics.totalUsers.toLocaleString('fr-FR')}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-600">CA Moyen/Client</span>
              <span className="font-semibold">
                {formatCurrency(data.statistics.totalRevenue / (data.statistics.totalUsers || 1))}
              </span>
            </div>
            <div className="pt-3 border-t">
              <div className="text-center">
                ...
                <div className="text-xs text-gray-500">Chiffre d'affaires total</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              ...
              <div>
                ...
                <div className="text-sm text-gray-600">{data.statistics.pendingOrders} en attente</div>
              </div>
            </Link>
            <Link 
              to="/commercial/stock" 
            ...
            >
              ...
              <div>
                ...
                <div className="text-sm text-gray-600">Vérifier les niveaux</div>
              </div>
            </Link>
