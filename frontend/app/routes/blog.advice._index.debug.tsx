...
export const meta: MetaFunction<typeof loader> = () => [
  ...
  { name: "description", content: "Découvrez nos conseils d'experts automobile." },
];
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  console.log("🔄 Loader advice - Début");
  
  try {
    const backendUrl = 'http://127.0.0.1:3000';
    const apiUrl = `${backendUrl}/api/blog/advice?page=${page}&limit=12`;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="bg-blue-900 text-white py-12">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-lg">Guides d'experts pour votre véhicule</p>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6 p-4 bg-gray-100 rounded-lg">
          <h2 className="font-bold mb-2">État du loader:</h2>
          <ul className="text-sm space-y-1">
            <li>✅ <strong>Succès:</strong> {data.success ? "OUI" : "NON"}</li>
            <li>📊 <strong>Articles:</strong> {data.articles.length}</li>
            <li>🔢 <strong>Total:</strong> {data.total}</li>
            <li>📄 <strong>Page:</strong> {data.page}/{data.totalPages}</li>
            {data.error && <li className="text-red-600">❌ <strong>Erreur:</strong> {data.error}</li>}
          </ul>
        </div>
        ...
        {data.articles.length === 0 && data.success && (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold mb-2">Aucun conseil trouvé</h3>
            <p className="text-gray-600">Les conseils seront bientôt disponibles.</p>
          </div>
        )}
