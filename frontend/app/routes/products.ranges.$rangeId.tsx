...
import { 
  ArrowLeft, Search, ChevronLeft, ChevronRight, Eye, 
  Package, Grid, List, RefreshCw, ShoppingCart, Star
} from 'lucide-react';
...
export async function loader({ request, params, context }: LoaderFunctionArgs) {
  try {
    ...
    const productsResponse = await fetch(`${baseUrl}/api/products/pieces-catalog?limit=1000`, {
      headers: { 
        'internal-call': 'true',
        'user-role': userRole,
    ...
    if (productsResponse.ok) {
      ...
      const productsForRange = Array.from({ length: Math.floor(Math.random() * 200) + 50 }, (_, i) => {
      ...
      let filteredProducts = productsForRange.filter((product) => {
        ...
        if (search) {
          ...
          const matchName = product.piece_name.toLowerCase().includes(searchTerm);
          const matchSku = product.piece_sku.toLowerCase().includes(searchTerm);
          const matchAlias = product.piece_alias && product.piece_alias.toLowerCase().includes(searchTerm);
        ...
        if (statusFilter === 'instock' && (!product.stock || product.stock <= 0)) return false;
      ...
      return json<ProductsByRangeData>({
        ...
        stats: {
          ...
          inStock: productsForRange.filter(p => p.stock && p.stock > 0).length,
...
export default function ProductsByRange() {
  ...
  const { user, range, products, stats, pagination, filters, enhanced, error } = data;
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <div className="flex items-center space-x-2 text-sm text-gray-600 mb-2">
            ...
            <span>/</span>
            <Link to="/products/ranges/advanced" className="hover:text-gray-900">Gammes</Link>
            <span>/</span>
                ...
                <Card key={product.piece_id} className="hover:shadow-lg transition-shadow">
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-2 mb-4">
                      ...
                      {user.role === 'pro' && product.price_pro && (
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-medium text-purple-600">{product.price_pro}€</span>
                        </div>
                      )}
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <Badge variant={
                          !product.stock || product.stock === 0 ? 'destructive' :
                    ...
                    <div className="flex gap-2">
                      ...
                      {product.piece_activ && product.stock && product.stock > 0 && (
            ...
            <Card className="mb-8">
              <CardContent className="p-0">
                <div className="divide-y">
                  {products.map((product) => (
                    <div key={product.piece_id} className="p-4 hover:bg-gray-50">
