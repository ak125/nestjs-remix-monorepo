...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    ...
    url.searchParams.getAll('marque').forEach(m => apiUrl.searchParams.append('marque', m));
    url.searchParams.getAll('gamme').forEach(g => apiUrl.searchParams.append('gamme', g));
    ...
    const groupedPieces = results.items ? groupSearchResultsByGamme(results.items, query) : [];
...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  ...
  return [
    { title: query ? `Recherche: ${query} | ${total} résultats` : 'Recherche pièces auto' },
    { name: 'description', content: query ? `${total} pièces auto pour "${query}" - Prix pas cher` : 'Recherchez des pièces automobiles par référence ou véhicule' },
...
export default function SearchPage() {
  ...
  const [allGroupedPieces, setAllGroupedPieces] = useState<GroupedSearchResults[]>(initialGroupedPieces);
  ...
  }, [query, initialPieces, initialGroupedPieces, results?.hasMore, results?.page]);
  ...
  useEffect(() => {
    if (fetcher.data && fetcher.data.pieces.length > 0) {
      setAllPieces(prev => {
        ...
        const newPieces = fetcher.data!.pieces.filter(p => !existingIds.has(p.id));
      ...
      setAllGroupedPieces(prev => {
        ...
        fetcher.data!.groupedPieces.forEach(newGroup => {
          ...
          if (existing) {
            ...
            const newItems = newGroup.items.filter(i => !existingIds.has(i.id));
        ...
        return Array.from(groupMap.values()).sort((a, b) => {
          if (normalizedQuery) {
            const aHasExact = a.items.some(item => 
              item.reference?.toUpperCase() === normalizedQuery
            );
            const bHasExact = b.items.some(item => 
              item.reference?.toUpperCase() === normalizedQuery
            );
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && hasMore && !isLoadingMore && fetcher.state === 'idle') {
  ...
  const filteredGroups = useMemo(() => {
    ...
    return allGroupedPieces.map(group => ({
      ...group,
      items: group.items.filter(piece => {
        ...
        if (activeFilters.quality !== 'all' && piece.quality !== activeFilters.quality) return false;
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {performance.totalItems !== undefined && performance.totalItems > 0 && (
                  <p className="text-sm text-gray-500">
                    <span className="font-semibold text-blue-600">{performance.totalItems.toLocaleString()}</span> résultats trouvés
                  </p>
                )}
              </div>
            </div>
            ...
            <aside className={`lg:col-span-1 ${showFilters ? 'block' : 'hidden lg:block'}`}>
              <div className="sticky top-4">
                <PiecesFilterSidebar
                  ...
                  getBrandCount={(brand) => dynamicFilterCounts.brandCounts.get(brand) || 0}
                  getQualityCount={(quality) => dynamicFilterCounts.qualityCounts.get(quality) || 0}
                  getPriceRangeCount={(range) => dynamicFilterCounts.priceCounts[range as 'low' | 'medium' | 'high'] || 0}
              ...
              <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-lg border border-gray-200/50 p-4 mb-6 sticky top-4 z-10">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2 bg-gradient-to-r from-blue-50 to-indigo-50 px-4 py-2 rounded-xl border border-blue-100">
                      ...
                      <span className="font-bold text-blue-700">{filteredProducts.length}</span>
                      <span className="text-gray-500 text-sm">pièces</span>
                    </div>
                    {query && (
                      <span className="text-sm text-gray-500">
                        pour "<span className="font-semibold text-blue-600">{query}</span>"
                      </span>
                    )}
                  </div>
                ...
                {results.suggestions && results.suggestions.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    <p className="text-sm text-gray-600 mb-2">💡 Essayez aussi :</p>
              ...
              {filteredGroups.length > 1 ? (
                ...
                <div className="space-y-8">
                  {filteredGroups.map((group, idx) => (
                    <div key={`${group.gammeName.toLowerCase()}-${idx}`} className="space-y-4">
                      ...
                      <div className="flex items-center gap-3 bg-gradient-to-r from-slate-800 via-slate-700 to-indigo-800 rounded-xl px-5 py-3.5 shadow-lg">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-white/60 text-xs">
                            {group.items.length} pièce{group.items.length > 1 ? 's' : ''} disponible{group.items.length > 1 ? 's' : ''}
                          </p>
                        </div>
                ...
                <div className="mt-6 bg-white/90 backdrop-blur-md rounded-2xl shadow-lg border border-indigo-200 p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {allPieces
                      ...
                      .map(piece => (
                        <div key={piece.id} className="bg-gray-50 rounded-xl p-4 relative">
                          <button
                          ...
                          <div className="font-bold text-gray-900">{piece.brand}</div>
                          <div className="text-sm text-gray-600">{piece.reference}</div>
              ...
              <div ref={loadMoreRef} className="py-8">
                {isLoadingMore && (
                  <div className="flex flex-col items-center justify-center gap-3">
                    ...
                    <p className="text-gray-500 text-sm">Chargement de plus de résultats...</p>
                  </div>
                )}
            ...
            <div className="max-w-lg mx-auto bg-white/80 backdrop-blur-md rounded-3xl shadow-xl border border-gray-200/50 p-8">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-100">
                <p className="text-sm text-blue-800">
                  💡 <strong>Astuce :</strong> Essayez "filtre huile renault clio" ou "WL7129"
                </p>
              </div>
            </div>
          </div>
