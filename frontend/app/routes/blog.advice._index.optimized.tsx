...
const Badge = ({ children, className = "", variant = "default", onClick }: { 
  ...
  return (
    <span 
      className={cn("inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border", variants[variant], className, onClick && "cursor-pointer")}
      onClick={onClick}
    >
      {children}
    </span>
...
// Loader optimisé avec gestion avancée
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    // Construction de l'URL API avec tous les paramètres
    ...
    const [adviceResponse, statsResponse] = await Promise.all([
      fetch(apiUrl.toString(), {
        headers: {
          ...
          'User-Agent': 'Remix-Advice-Client/1.0',
        },
    ...
    let data: LoaderData = {
      ...
      popularKeywords: [],
      stats: { totalAdvice: 0, totalViews: 0, avgReadingTime: 5 },
    };
    ...
    if (statsResponse?.ok) {
      ...
      if (statsData.success) {
        ...
        data.popularKeywords = statsData.data.topKeywords?.slice(0, 10) || [];
      }
    }

    // Gammes simulées (adaptez selon votre API)
    data.gammes = [
      { id: "entretien", name: "Entretien", code: "entretien", description: "Guides d'entretien régulier", articlesCount: 25 },
      ...
      { id: "electrique", name: "Système Électrique", code: "electrique", description: "Électronique automobile", articlesCount: 15 },
    ];
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    switch (actionType) {
      case "bookmark":
        // Logique pour bookmarker un conseil
        return json({ success: true, message: "Conseil ajouté aux favoris" });
      
      case "share":
        // Logique pour partager un conseil
...
export default function AdviceListPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
      ...
      <motion.section 
        className="bg-gradient-to-br from-blue-900 via-blue-800 to-purple-900 text-white py-16 relative overflow-hidden"
              ...
              <Form method="get" className="flex flex-col md:flex-row gap-4">
                ...
                <Select 
                  ...
                  onValueChange={(value) => {
                    ...
                    handleFilterChange("gamme", value);
                  }}
                >
                  <SelectTrigger className="w-full md:w-64 py-4 bg-white/90 border-0 text-gray-900">
                    <SelectValue placeholder="Toutes les gammes" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="">Toutes les gammes</SelectItem>
                    {data.gammes.map((gamme) => (
                      <SelectItem key={gamme.id} value={gamme.code}>
                ...
                <Button 
                  ...
                  size="lg" 
                  disabled={isLoading}
                  className="bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 px-8 py-4 rounded-xl text-lg font-semibold group"
                >
                  {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {data.gammes.slice(0, 4).map((gamme) => (
                          <Button
                            key={gamme.id}
                            variant={selectedGamme === gamme.code ? "default" : "ghost"}
                            size="sm"
          ...
          <main className="lg:col-span-3">
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="difficulty_asc">Difficulté croissante</SelectItem>
                    <SelectItem value="difficulty_desc">Difficulté décroissante</SelectItem>
                    <SelectItem value="reading_time">Temps de lecture</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <AnimatePresence mode="wait">
              <motion.div
                key={`${data.page}-${sortBy}-${selectedGamme}`}
              ...
              >
                {data.articles.map((article, index) => (
                  <motion.div
                    ...
                    whileHover={{ y: -4, scale: viewMode === 'grid' ? 1.02 : 1.01 }}
                    ...
                    {viewMode === 'grid' ? (
                      ...
                      <Card className="h-full group hover:shadow-xl transition-all duration-300 border-0 bg-white/80 backdrop-blur-sm overflow-hidden">
                        <CardHeader className="pb-3">
                          ...
                          <CardTitle className="text-lg group-hover:text-blue-600 transition-colors line-clamp-2 leading-tight">
                            {article.title}
                          </CardTitle>
                        </CardHeader>
