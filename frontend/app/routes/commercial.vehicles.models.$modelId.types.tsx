    ...
    if (types.length > 0) {
      ...
      if (firstType.type_marque_id) {
        ...
        const brandResponse = await fetch(
          `${baseUrl}/api/vehicles/brands/${firstType.type_marque_id}/models?limit=100`,
          {
            headers: { "internal-call": "true" },
          },
        );
        ...
        if (brandResponse.ok) {
          ...
          const modelInfo = brandData.data?.find(
            (m: any) => m.modele_id == parseInt(modelId),
          );
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <h1 className="text-2xl font-bold text-gray-900">
                Types - {model?.brand_name}{" "}
                {model?.modele_name || `Modèle #${params.modelId}`}
              </h1>
              <p className="text-gray-600 mt-1">
                {types.length} motorisation{types.length > 1 ? "s" : ""}{" "}
                disponible{types.length > 1 ? "s" : ""}
              </p>
            </div>
                ...
                {types.map((type) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="info">{type.type_fuel}</Badge>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <CardContent>
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">
                  Aucun type trouvé pour ce modèle
                </p>
              </div>
            </CardContent>
          )}
