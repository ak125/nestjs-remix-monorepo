...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    try {
      ...
      bodyText = await Promise.race([
        ...
        new Promise<string>((_, reject) => 
          setTimeout(() => reject(new Error('⏱️ Timeout reading body (3s)')), timeoutMs)
        ),
      ]);
      ...
      return json({ error: 'Timeout or error reading request body' }, { status: 408 });
    }
    ...
    if (!acceptTerms) {
      ...
      return json({ error: 'Vous devez accepter les conditions générales' }, { status: 400 });
    }
    ...
    console.log('🔍 Récupération commande depuis:', `${backendUrl}/api/orders/${orderId}`);
    ...
    console.log('✅ Paiement initialisé, transactionId:', paymentData.transactionId);
    ...
    if (paymentMethod === 'cyberplus' && paymentData.formData && paymentData.gatewayUrl) {
      ...
      console.log('📋 Form data keys:', Object.keys(paymentData.formData).join(', '));
