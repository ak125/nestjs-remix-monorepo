...
function familleColorToTheme(
  ...
  if (
    familleColor.includes("red") ||
    familleColor.includes("rose") ||
    familleColor.includes("orange-6")
  ) {
    return "braking"; // Rouge/Orange = Freinage
  }
  if (
    familleColor.includes("slate") ||
    familleColor.includes("gray") ||
    familleColor.includes("neutral")
  ) {
  ...
  if (
    familleColor.includes("emerald") ||
    familleColor.includes("green") ||
    familleColor.includes("lime")
  ) {
    return "engine"; // Vert = Moteur/Alimentation
  }
  if (
    familleColor.includes("cyan") ||
    familleColor.includes("sky") ||
    familleColor.includes("teal")
  ) {
...
export function ProductGuideAccordion({
  ...
  const guideTitle =
    title ||
    `Comment choisir ${productName ? `vos ${productName.toLowerCase()}` : "vos pièces"} ?`;
    ...
    <section className={`container mx-auto px-4 relative z-10 ${className}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative bg-white rounded-3xl shadow-2xl border border-neutral-100 overflow-hidden">
          ...
          <div
            className={`relative bg-gradient-to-br ${theme.header} px-6 py-10 md:px-10 md:py-12 border-b border-neutral-100`}
          >
            <div className="max-w-3xl mx-auto text-center">
              ...
              <div
                className={`inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r ${theme.badge} rounded-full shadow-lg mb-3`}
              >
                <svg
              ...
              <p className="text-base md:text-lg text-slate-600 font-medium max-w-2xl mx-auto">
                Suivez nos 3 étapes simples pour choisir vos équipements en
                toute confiance
              </p>
            </div>
          </div>
            ...
            {sections.map((section, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-blue-500/0 to-transparent ${color.line} transition-all duration-500`}
                  />
                  ...
                  <div className="flex items-center gap-4 mb-7">
                    <div className="relative flex-shrink-0">
                      ...
                      <div
                        className={`absolute inset-0 bg-gradient-to-br ${color.badge} rounded-2xl blur-lg ${color.badgeBg} group-hover:opacity-60 transition-opacity duration-500`}
                      />

                      {/* Badge numéro avec couleur */}
                      <div
                        className={`relative w-14 h-14 rounded-2xl bg-gradient-to-br ${color.badge} text-white flex items-center justify-center font-bold text-2xl shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-500`}
                      >
                        {index + 1}
                      </div>
                    </div>
                  ...
                  <ul className="space-y-5">
                    {section.items.map((item, itemIndex) => (
                      ...
                      >
                        ...
                        <span
                          className={`flex-shrink-0 mt-0.5 w-5 h-5 flex items-center justify-center rounded-lg ${color.iconBg} ${color.iconText} ${color.iconHover} group-hover/item:scale-110 transition-all duration-300`}
                        >
                          {item.icon}
                        </span>
                  ...
                  <div
                    className={`absolute bottom-0 left-0 right-0 h-1.5 bg-gradient-to-r ${color.bar} transform scale-x-0 group-hover:scale-x-100 transition-transform duration-700 origin-left`}
                  />
                </div>
              );
            })}
