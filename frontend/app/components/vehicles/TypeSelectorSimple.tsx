        ...
        <div className="mt-2 text-xs text-gray-600 space-y-1">
          ...
          <div className="grid grid-cols-2 gap-2">
            {selectedType.type_engine_code && (
              <span><strong>Code moteur:</strong> {selectedType.type_engine_code}</span>
            )}
            {selectedType.type_fuel && (
              <span><strong>Carburant:</strong> {selectedType.type_fuel}</span>
            )}
            {selectedType.type_liter && (
              <span><strong>Cylindrée:</strong> {selectedType.type_liter}cc</span>
            )}
            {selectedType.type_power_ps && (
              <span><strong>Puissance:</strong> {selectedType.type_power_ps}cv</span>
            )}
            {selectedType.type_power_kw && (
              <span><strong>Puissance:</strong> {selectedType.type_power_kw}kW</span>
            )}
            {selectedType.type_year_from && (
              <span><strong>Période:</strong> {selectedType.type_year_from}
