...
export interface ConversionButtonProps extends Omit<
...
const VARIANT_CLASSES = {
  ...
  primary:
    "bg-gradient-to-r from-primary-500 to-orange-500 text-white hover:from-primary-600 hover:to-orange-600 active:from-primary-700 active:to-orange-700 shadow-lg hover:shadow-xl active:shadow-md",

  // Secondary: Bleu foncé = Action secondaire (confiance)
  secondary:
    "bg-secondary-500 text-white hover:bg-secondary-700 active:bg-secondary-800 shadow-md hover:shadow-lg active:shadow-sm",

  // Ghost: Transparent = Action tertiaire (non-intrusif)
  ghost:
    "bg-transparent text-secondary-500 border-2 border-secondary-500 hover:bg-secondary-500 hover:text-white active:bg-secondary-700",

  // Urgent: Rouge pulsant = Urgence maximale (stock faible)
  urgent:
    "bg-red-700 text-white hover:bg-red-800 active:bg-red-900 shadow-xl animate-pulse-soft",
} as const;
...
export function ConversionButton({
  ...
  const handleClick = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // Analytics tracking
    const label =
      trackingLabel || (typeof children === "string" ? children : "CTA Click");
  ...
  // ========================================
  const baseClasses =
    "font-heading font-bold rounded-lg transition-all duration-300 ease-out flex items-center justify-center focus:outline-none focus:ring-4 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed";
  ...
  const successClasses = showSuccess ? "bg-semantic-success scale-105" : "";
...
export function PrimaryCTA(
  props: Omit<ConversionButtonProps, "variant" | "size">,
) {
  return (
    <ConversionButton
      variant="primary"
...
export function UrgentCTA(props: Omit<ConversionButtonProps, "variant">) {
  return <ConversionButton variant="urgent" showSuccessAnimation {...props} />;
}

/** CTA mobile - Optimisé tactile */
export function MobileCTA(
  props: Omit<ConversionButtonProps, "size" | "fullWidthOnMobile">,
) {
  return (
    <ConversionButton size="large" fullWidthOnMobile breathing {...props} />
  );
}

/** CTA comparaison - Action secondaire */
export function SecondaryCTA(props: Omit<ConversionButtonProps, "variant">) {
  return <ConversionButton variant="secondary" {...props} />;
}

/** CTA discret - Action tertiaire */
export function GhostCTA(
  props: Omit<ConversionButtonProps, "variant" | "breathing">,
) {
  return <ConversionButton variant="ghost" breathing={false} {...props} />;
}
...
function LoadingSpinner({ size }: { size: ConversionButtonProps["size"] }) {
  const spinnerSize =
    size === "hero" ? "w-6 h-6" : size === "large" ? "w-5 h-5" : "w-4 h-4";
