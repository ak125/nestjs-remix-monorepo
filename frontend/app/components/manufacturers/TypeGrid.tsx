  ...
  if (layout === "list") {
    return (
      <div className="space-y-4">
        {types.map((type) => (
          <Link key={type.id} to={`${linkPrefix}/${type.id}`} className="block">
                    ...
                    {(type.body_type || type.segment) && (
                      <div className="mt-4 pt-4 border-t border-gray-100">
                        <div className="flex gap-4 text-sm text-gray-600">
                          {type.body_type && (
                            <span>Style: {type.body_type}</span>
                          )}
              ...
              {type.models_count && type.models_count > 0 && (
                <div className="mt-4 pt-4 border-t text-center">
                  <span className="text-sm text-blue-600 font-medium">
                    {type.models_count} modèle{type.models_count > 1 ? "s" : ""}{" "}
                    disponible{type.models_count > 1 ? "s" : ""}
                  </span>
                </div>
              )}
            </CardContent>
