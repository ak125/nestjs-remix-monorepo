...
export const VehicleAnalytics = memo(function VehicleAnalytics({
  vehicle,
  userId,
}: VehicleAnalyticsProps) {
  const trackEvent = useCallback(
    (eventType: AnalyticsEvent["eventType"], additionalData?: any) => {
      ...
      if (process.env.NODE_ENV === "development") {
        logger.log("🔍 Vehicle Analytics Event:", event);
      }
    },
    [vehicle, userId],
  );
  ...
  const sendAnalyticsEvent = async (event: AnalyticsEvent) => {
    try {
      ...
      logger.error("Erreur envoi analytics:", error);
    }
  };
