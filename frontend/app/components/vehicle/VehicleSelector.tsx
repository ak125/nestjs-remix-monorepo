...
interface VehicleSelectorProps {
  ...
  className?: string;
  // 🎯 Nouvelles props pour redirection contextuelle
  redirectMode?: 'vehicle-catalog' | 'current-category' | 'custom';
  currentCategory?: string; // slug de la catégorie actuelle (ex: "filtre-a-huile")
}
...
export default function VehicleSelector({
  ...
  // 📢 Notification sélection au parent + Navigation contextuelle
  useEffect(() => {
    ...
    // 🎯 Navigation automatique si tous les éléments sont sélectionnés et navigateOnSelect=true
    if (navigateOnSelect && selectedBrand && selectedModel && selectedType) {
      // Construire les slugs pour l'URL
      const brandName = selectedBrand.marque_alias || selectedBrand.marque_name || 'marque';
      const modelName = selectedModel.modele_alias || selectedModel.modele_name || 'modele';
      
      // Simplifier la motorisation
      let typeDesc = selectedType.type_alias || '';
      if (!typeDesc && selectedType.type_liter && selectedType.type_fuel) {
        const liter = (parseInt(selectedType.type_liter) / 100).toFixed(1);
        const fuel = selectedType.type_fuel.toLowerCase();
        typeDesc = `${liter}-${fuel}`;
      }

      let targetUrl = '';

      if (redirectMode === 'vehicle-catalog') {
        // Mode 1: Redirection vers le catalogue complet du véhicule
        const brandSlug = `${brandName}-${selectedBrand.marque_id}`;
        const modelSlug = `${modelName}-${selectedModel.modele_id}`;
        const typeSlug = `${typeDesc || 'type'}-${selectedType.type_id}.html`;
        targetUrl = `/constructeurs/${brandSlug}/${modelSlug}/${typeSlug}`;
      } else if (redirectMode === 'current-category' && currentCategory) {
        // Mode 2: Redirection vers la catégorie actuelle filtrée pour ce véhicule
        const vehicleSlug = `${brandName}-${modelName}-${typeDesc}`.toLowerCase()
          .replace(/\s+/g, '-')
          .replace(/[^\w-]/g, '')
          .replace(/--+/g, '-');
        targetUrl = `/pieces/${currentCategory}/vehicule/${vehicleSlug}`;
      }

      if (targetUrl) {
        console.log('🎯 Navigation contextuelle vers:', targetUrl);
        console.log('📝 Mode:', redirectMode, 'Catégorie:', currentCategory);
        
        // Délai de 1.5 secondes pour laisser l'utilisateur voir la sélection
        setTimeout(() => {
          navigate(targetUrl);
        }, 1500);
      }
    }
  }, [selectedBrand, selectedYear, selectedModel, selectedType, onVehicleSelected, navigateOnSelect, redirectMode, currentCategory, navigate]);
