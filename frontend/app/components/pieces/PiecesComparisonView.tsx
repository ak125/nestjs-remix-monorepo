          ...
          <table className="min-w-full divide-y divide-gray-200">
                ...
                {comparedPieces.map((piece) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-gray-500 mt-4">
                      Pièce {comparedPieces.indexOf(piece) + 1}
                    </div>
                  </th>
                ))}
              </tr>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {comparisonRows.map((row, rowIndex) => (
                ...
                >
                  ...
                  {comparedPieces.map((piece) => (
                    <td
                      key={piece.id}
                      className="px-4 py-4 text-sm text-gray-700 text-center align-middle"
                    >
              ...
              <tr className="bg-gray-50">
                ...
                {comparedPieces.map((piece) => {
                  const hasStock =
                    piece.stock === "En stock" || piece.stock === "available";
                  return (
        ...
        <div className="mt-4 p-4 bg-primary/5 rounded-lg border border-blue-200">
          <div className="flex items-center justify-between">
            <div className="text-sm text-blue-900">
              ...
              {Math.min(...comparedPieces.map((p) => p.price)).toFixed(2)}€
            </div>
            <div className="text-sm text-blue-900">
              <strong>Prix maximum:</strong>{" "}
              {Math.max(...comparedPieces.map((p) => p.price)).toFixed(2)}€
            </div>
            <div className="text-sm text-blue-900">
              <strong>Différence:</strong>{" "}
              {(
                Math.max(...comparedPieces.map((p) => p.price)) -
                Math.min(...comparedPieces.map((p) => p.price))
              ).toFixed(2)}
              €
            </div>
          </div>
...
function renderComparisonCell(
  ...
  switch (key) {
    ...
    case "brand":
      return <Badge variant="info">{piece.brand}</Badge>;
    ...
    case "price":
      ...
      return (
        <div
          className={`font-bold text-lg ${isCheapest ? "text-green-600" : "text-gray-900"}`}
        >
