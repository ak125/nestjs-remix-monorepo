...
const RELIABILITY_COLORS = [
  ...
  {
    min: 3,
    gradient: "from-orange-400 via-rose-500 to-red-400",
    text: "text-rose-600",
  },
  {
    min: 0,
...
const PieceCard = memo(function PieceCard({
  ...
  // FIX 2026-01-17: Masquer fiabilité si valeur par défaut (stars=3 ou undefined)
  const { reliability, colors, hasReliableStars } = useMemo(() => {
    const hasReliable = piece.stars !== undefined && piece.stars !== 3;
    const stars = piece.stars || 3;
    const rel = Math.round((stars / 6) * 10);
    return {
      reliability: rel,
      colors: getReliabilityColors(rel),
      hasReliableStars: hasReliable,
    };
  }, [piece.stars]);
      ...
      <div className="text-center py-20 bg-gradient-to-br from-background via-muted/50 to-muted rounded-2xl border-2 border-dashed border-border relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-md mx-auto px-4">
          ...
          <p className="text-muted-foreground mb-6 leading-relaxed">
            Essayez de modifier vos filtres ou d'élargir vos critères de
            recherche pour découvrir plus de résultats.
          </p>
          <div className="inline-flex flex-col gap-2 text-sm text-left bg-card rounded-xl p-4 shadow-sm border border-border">
            <div className="flex items-start gap-2">
              ...
              <span className="text-foreground">
                Vérifiez vos filtres de marque et qualité
              </span>
            </div>
            <div className="flex items-start gap-2">
              <svg
