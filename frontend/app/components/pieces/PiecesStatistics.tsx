...
export const PiecesStatistics = memo(function PiecesStatistics({
  ...
  const stats = React.useMemo(() => {
    ...
    const hasStockCount = pieces.filter(
      (p) => p.stock === "En stock" || p.stock === "available",
    ).length;
              ...
              <div>
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="text-gray-600">
                    Moins de 50€ (économique)
                  </span>
                  <span className="font-medium text-green-700">
                    {stats.priceDistribution.lowPrice} pièces
                  </span>
                </div>
                <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${(stats.priceDistribution.lowPrice / stats.total) * 100}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between text-xs mb-1">
                  ...
                  <span className="font-medium text-blue-700">
                    {stats.priceDistribution.midPrice} pièces
                  </span>
                </div>
                <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${(stats.priceDistribution.midPrice / stats.total) * 100}%`,
                    }}
                  />
                </div>
              </div>
        ...
        <div className="bg-gradient-to-br from-orange-50 to-red-50 rounded-lg p-5 border border-orange-200">
          ...
          <ul className="space-y-2 text-sm text-orange-800">
            <li className="flex items-start gap-2">
              ...
              <span>
                Notre catalogue contient{" "}
                <strong>{stats.brandsCount} marques différentes</strong>, vous
