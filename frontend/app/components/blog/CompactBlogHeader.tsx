...
export function CompactBlogHeader({
  ...
  return (
    <section className={`relative bg-gradient-to-r ${gradientFrom} ${gradientTo} text-white py-4 md:py-6 ${className}`}>
      <div className="container mx-auto px-4">
        <div className="max-w-7xl mx-auto">
          {/* Breadcrumb */}
          {breadcrumbItems.length > 0 && (
            <nav className="flex items-center gap-2 text-blue-50 text-xs mb-3" aria-label="Breadcrumb">
              {breadcrumbItems.map((item, index) => (
                <React.Fragment key={index}>
                  ...
                  {item.href ? (
                    ...
                    <span className={index === breadcrumbItems.length - 1 ? "text-white font-medium" : ""}>
                      {item.label}
                    </span>
                  )}
                </React.Fragment>
