...
export function OwnerActionQueue({ data }: { data: CommandCenterResponse }) {
  ...
  for (const a of data.alerts) sevByCode.set(a.code + ":" + a.target_id, a.severity);

  const actions = [...data.owner_actions]
    .map((a) => ({ ...a, severity: sevByCode.get(a.from_alert + ":" + a.target_id) ?? "info" }))
  ...
  return (
    <Card>
      ...
      <CardContent>
        {actions.length === 0 ? (
          <Alert variant="success" icon={<CheckCircle2 className="h-4 w-4" aria-hidden />}>
            <AlertDescription>Aucune action en attente — opérations alignées.</AlertDescription>
          </Alert>
        ) : (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{a.decision}</p>
                ...
                <div className="flex shrink-0 flex-wrap items-center gap-2 pl-7 sm:pl-0">
                  <Badge variant={severityVariant[a.severity] ?? "info"}>{a.from_alert}</Badge>
                  <Badge variant="outline" aria-label={`Responsable : ${a.owner}`}>
                    {a.owner}
                  </Badge>
                </div>
              </li>
