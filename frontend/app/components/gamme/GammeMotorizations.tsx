...
export default function GammeMotorizations({
  ...
  return (
    <Section variant="slate" id="compat" className="scroll-mt-16">
      <SectionHeader
        title={h2Override || "Compatibilités"}
        sub={intro}
        trailing={
          <Badge
            variant="outline"
      ...
      <div className="flex flex-col gap-2.5 lg:grid lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 lg:gap-3">
        {visible.map((m, i) => (
          <Reveal
          ...
          >
            <Link
              to={m.link || "#"}
              className="bg-white border border-slate-200 rounded-[22px] p-4 shadow-[0_6px_18px_rgba(15,23,42,0.05)] cursor-pointer hover:shadow-lg hover:border-blue-100 hover:-translate-y-0.5 transition-all duration-200 group block"
            >
              <div className="flex items-start justify-between mb-2.5">
                <div>
                  ...
                  <div className="text-[14px] font-semibold text-slate-800 mt-0.5 font-heading group-hover:text-blue-700 transition-colors">
                    {m.modele_name}
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-[14px] font-bold text-slate-800 font-heading">
                    {m.type_name}
                  </div>
                  {m.puissance && (
      ...
      {filtered.length === 0 && search && (
        <div className="py-8 text-center animate-subtle-fade-in">
