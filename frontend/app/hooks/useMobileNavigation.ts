...
export function useMobileNavigation() {
  ...
  const detectDeviceCapabilities = useCallback(() => {
    ...
    if (isMobile) {
      preferredNavigation =
        orientation === "portrait" ? "bottom-tabs" : "drawer";
    } else if (isTablet) {
  ...
  return {
    ...
    isSmallScreen:
      state.isMobile || (state.isTablet && state.orientation === "portrait"),
    ...
    // Métriques utiles
    availableHeight:
      state.screenHeight - getSafeAreaInsets().top - getSafeAreaInsets().bottom,
    availableWidth:
      state.screenWidth - getSafeAreaInsets().left - getSafeAreaInsets().right,
  };
}
