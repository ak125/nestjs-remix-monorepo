...
export function getBrandLogoUrl(brandName: string): string {
  const normalizedName =
    BRAND_LOGO_MAPPING[brandName.toUpperCase()] ||
    brandName.toLowerCase().replace(/\s+/g, "-");

  // ✅ Migration /img/* : Proxy Caddy avec cache 1 an
  return `/img/uploads/constructeurs-automobiles/marques-logos/${normalizedName}.webp`;
}
...
export const useImagePreloader = () => {
  ...
  const preloadVisibleBrands = useCallback(
    async (brands: Brand[], limit = 20) => {
      const visibleBrands = brands.slice(0, limit);
      const urls = visibleBrands.map((brand) =>
        getBrandLogoUrl(brand.marque_name),
      );
