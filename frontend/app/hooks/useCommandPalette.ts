...
export function useCommandPalette() {
  ...
  useEffect(() => {
    try {
      ...
      logger.warn("Erreur lors du chargement des actions récentes:", error);
    }
  }, []);
  ...
  const saveRecentActions = useCallback((actions: RecentAction[]) => {
    ...
    scheduleIdleCallback(() => {
      try {
        ...
        logger.warn(
          "Erreur lors de la sauvegarde des actions récentes:",
          error,
        );
      }
    });
  ...
  const addRecentAction = useCallback(
    (action: { id: string; label: string; href: string }) => {
      const now = Date.now();
  ...
  const trackAction = useCallback(
    (
      actionId: string,
      actionType: "navigation" | "search" | "shortcut" = "navigation",
    ) => {
      // Ici on pourrait intégrer avec un service d'analytics
      logger.log(`Command Palette Action: ${actionId} (${actionType})`);
    },
    [],
  );
