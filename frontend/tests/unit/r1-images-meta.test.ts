...
// Helper: simule la logique OG du meta() de pieces.$slug.tsx
function resolveOgImage(
  r1Images: R1ImageItem[],
  pgPic: string | undefined,
): string {
  const r1OgSlot = r1Images.find((i) => i.slot === "OG");
  const r1HeroSlot = r1Images.find((i) => i.slot === "HERO");
...
describe("R1 OG meta fallback", () => {
  it("1. OG slot present → ogImage uses OG path", () => {
    const ogImage = resolveOgImage([OG_IMG, HERO_IMG], "/img/pg.webp");
  ...
  it("2. No OG, HERO present → ogImage uses HERO path", () => {
    const ogImage = resolveOgImage([HERO_IMG], "/img/pg.webp");
  ...
  it("5. og:image:alt uses slot alt, not title", () => {
    const alt = resolveOgAlt([OG_IMG], "Generic Page Title");
    expect(alt).toBe("Filtre OG alt");

    const altHero = resolveOgAlt([HERO_IMG], "Generic Page Title");
