...
# Compte les docs avec truth_level non null (= admissibles)
ADMISSIBLE=$(echo "$RESPONSE" | jq '[if type == "array" then .[] elif .data then .data[] else empty end | select(.truth_level != null)] | length' 2>/dev/null)
