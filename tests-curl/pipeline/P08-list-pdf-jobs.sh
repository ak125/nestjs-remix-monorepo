#!/bin/bash
# PIPELINE: Lister jobs PDF (read-only, safe)
BASE_URL="${BASE_URL:-http://localhost:3000}"
COOKIE_FILE="$(dirname "$0")/../.cookies"
TEST_NAME="pipeline/list-pdf-jobs"

RESPONSE=$(curl -s -b "$COOKIE_FILE" "$BASE_URL/api/rag/admin/ingest/pdf/jobs")
...
if [ "$HTTP_CODE" = "200" ]; then
  ...
  echo "PASS: $TEST_NAME (HTTP 200, $COUNT PDF jobs)"
else
